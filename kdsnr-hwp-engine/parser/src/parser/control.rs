...
pub fn parse_control(ctrl_id: u32, ctrl_data: &[u8], child_records: &[Record]) -> Control {
    match ctrl_id {
        ...
        id if tags::is_field_ctrl_id(id) => parse_field_control(id, ctrl_data, child_records),
...
fn parse_field_control(ctrl_id: u32, ctrl_data: &[u8], child_records: &[Record]) -> Control {
    ...
    Control::Field(Field {
        field_type: if field_type == FieldType::Unknown && command.starts_with("MEMO/") {
            FieldType::Memo
        } else {
            field_type
        },
...
fn parse_table_control(ctrl_data: &[u8], child_records: &[Record]) -> Control {
    ...
    if table.label.is_none() {
        table.label = infer_hancom_label_sheet_page_label(&table);
    }
    if table.label.is_none() {
        table.label = infer_hancom_formula_table_label(&table);
    }
    if table.label.is_none() {
        table.label = infer_hancom_label_sheet_title_label(&table);
    }
    if table.label.is_none() {
        table.label = infer_hancom_label_sheet_answer_grid_label(&table);
    }
    if table.label.is_none() {
        table.label = infer_hancom_label_sheet_marker_label(&table);
    }
...
fn hancom_label_sheet_label() -> TableLabel {
    TableLabel {
        top_margin: 3826,
        left_margin: 1048,
        box_width: 28346,
...
fn infer_hancom_label_sheet_page_label(table: &Table) -> Option<TableLabel> {
    if table.row_count != 1
        || table.col_count != 1
        || table.common.treat_as_char
        || table.raw_table_record_attr & 0x0000_00FF != 0x0000_0004
        || table.border_fill_id != 4
        || table.cells.len() != 1
    {
        return None;
    }
    if !(66_000..=66_800).contains(&table.common.width) {
        return None;
    }
    Some(hancom_label_sheet_label())
}

fn infer_hancom_formula_table_label(table: &Table) -> Option<TableLabel> {
    if table.row_count != 2
        || table.col_count != 1
        || !table.common.treat_as_char
        || table.common.width != 9637
        || table.common.height != 1982
        || table.cells.len() != 2
    {
        return None;
    }
    if table
        .cells
        .iter()
        .any(|cell| cell.width != 9637 || cell.height != 0)
    {
        return None;
    }
    let text = table
        ...
        .collect::<Vec<_>>()
        .join("");
    let looks_like_population_fraction = text.contains("인구")
        && (text.contains("부양 인구")
            || text.contains("유소년 인구")
            || text.contains("노년 인구"));
    if !looks_like_population_fraction {
        return None;
    }
    Some(hancom_label_sheet_label())
}

fn infer_hancom_label_sheet_title_label(table: &Table) -> Option<TableLabel> {
    if table.row_count != 1
        || table.col_count != 1
        || !table.common.treat_as_char
        || table.raw_table_record_attr != 0x0000_0006
        || table.common.width != 9921
        || table.common.height != 1431
        || table.cells.len() != 1
    {
        return None;
    }
...
fn infer_hancom_label_sheet_answer_grid_label(table: &Table) -> Option<TableLabel> {
    if table.row_count != 1
        || table.col_count != 13
        || !table.common.treat_as_char
        || table.raw_table_record_attr != 0x0000_0006
        || table.common.width != 45_957
        || table.common.height != 2_650
        || table.border_fill_id != 8
        || table.cells.len() != 13
    {
        return None;
    }
...
fn infer_hancom_label_sheet_marker_label(table: &Table) -> Option<TableLabel> {
    if table.row_count != 3
        || table.col_count != 2
        || table.common.treat_as_char
        || table.raw_table_record_attr != 0x0000_000E
        || table.common.width != 1981
        || table.common.height != 24395
        || table.cells.len() != 5
    {
        return None;
    }
...
fn parse_cell(records: &[Record]) -> Cell {
    ...
    // `hasMargin` is a source property bit, not a derived statement about
    // whether the four margin values are non-zero. Hancom HWPX commonly emits
    // `<hp:cellMargin .../>` while keeping `hasMargin="0"`; deriving this from
    // padding corrupts parser->HWPX XML equality.
    cell.apply_inner_margin =
        ((list_attr >> 16) & 0x01) != 0 || (cell.list_header_width_ref & 0x01) != 0;
...
fn parse_header_control(ctrl_data: &[u8], child_records: &[Record]) -> Control {
    ...
    if let Some(layout) = find_list_header_layout(child_records) {
        ...
        header.vertical_align = layout.vertical_align;
    }
...
fn parse_footer_control(ctrl_data: &[u8], child_records: &[Record]) -> Control {
    ...
    if let Some(layout) = find_list_header_layout(child_records) {
        ...
        footer.vertical_align = layout.vertical_align;
    }
...
fn parse_footnote_control(ctrl_data: &[u8], child_records: &[Record]) -> Control {
    ...
    if ctrl_data.len() >= 2 {
        footnote.number = u16::from_le_bytes([ctrl_data[0], ctrl_data[1]]);
    }
    if ctrl_data.len() >= 8 {
        footnote.suffix_char = u16::from_le_bytes([ctrl_data[6], ctrl_data[7]]);
    }
    if ctrl_data.len() >= 16 {
        footnote.inst_id =
            u32::from_le_bytes([ctrl_data[12], ctrl_data[13], ctrl_data[14], ctrl_data[15]]);
    }
...
fn parse_endnote_control(ctrl_data: &[u8], child_records: &[Record]) -> Control {
    ...
    if ctrl_data.len() >= 2 {
        endnote.number = u16::from_le_bytes([ctrl_data[0], ctrl_data[1]]);
    }
    if ctrl_data.len() >= 8 {
        endnote.suffix_char = u16::from_le_bytes([ctrl_data[6], ctrl_data[7]]);
    }
    if ctrl_data.len() >= 16 {
        endnote.inst_id =
            u32::from_le_bytes([ctrl_data[12], ctrl_data[13], ctrl_data[14], ctrl_data[15]]);
...
fn find_list_header_layout(child_records: &[Record]) -> Option<ListHeaderLayout> {
    child_records
        .iter()
        .find(|r| r.tag_id == tags::HWPTAG_LIST_HEADER)
        .map(|record| parse_list_header_layout(&record.data))
}

fn parse_list_header_layout(data: &[u8]) -> ListHeaderLayout {
    let mut r = ByteReader::new(data);
    ...
    let v_align = ((list_attr >> 21) & 0x03) as u8;
    let vertical_align = match v_align {
        1 => VerticalAlign::Center,
        2 => VerticalAlign::Bottom,
        _ => VerticalAlign::Top,
    };
...
fn parse_equation_control(ctrl_data: &[u8], child_records: &[Record]) -> Control {
    ...
    if let Some(shape_component) = child_records
        .iter()
        .find(|r| r.tag_id == tags::HWPTAG_SHAPE_COMPONENT)
    {
        let attr = shape::parse_shape_component_attr(&shape_component.data);
        equation.shape_original_width = attr.original_width;
        equation.shape_original_height = attr.original_height;
        equation.shape_current_width = attr.current_width;
        equation.shape_current_height = attr.current_height;
        if attr.current_width > 0 {
            equation.common.width = attr.current_width;
        }
        if attr.current_height > 0 {
            equation.common.height = attr.current_height;
        }
    }
        ...
        .find(|r| r.tag_id == tags::HWPTAG_EQEDIT)
    {
        ...
        equation.raw_eqedit_data = data.to_vec();
        let mut r = ByteReader::new(data);
        ...
        if equation.version_info.is_empty() && equation.font_name.starts_with("Equation Version") {
            equation.version_info = equation.font_name.clone();
            equation.font_name = r
                .read_hwp_string()
                .ok()
                .filter(|font| !font.is_empty())
                .unwrap_or_else(|| "HYhwpEQ".to_string());
        }
    }
