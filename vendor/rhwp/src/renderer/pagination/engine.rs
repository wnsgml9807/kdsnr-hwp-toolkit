...
impl Paginator {
    ...
    pub fn paginate_with_measured_opts(
        ...
        for (para_idx, para) in paragraphs.iter().enumerate() {
            ...
            // 2026-05-19: wrap=TopAndBottom 비-TAC 표 (+ vert_offset=0) 보유 paragraph 의
            // para_height 를 stored vp gap 으로 override.
            // 한컴 spec: 텍스트가 표 영역 옆/위/아래에 wrap 되어 paragraph 의 stored gap =
            // 표 영역 (= 표 + outer_bottom). text 의 lh+ls 는 추가 차지 없음.
            // rhwp 기존: paragraph_height (text 만) + table_total_height (표 + host_spacing)
            // 합산 → text 영역만큼 중복. paginate_table_control 의 누적도 같이 skip 한다.
            // 인접 paragraph 의 first.vp 가 양수일 때만 (section-cumulative 보장).
            let has_non_tac_top_bottom_zero_offset = para.controls.iter().any(|c| {
            ...
            if has_non_tac_top_bottom_zero_offset {
                if let (Some(next_first), Some(cur_first)) = (
                    paragraphs
                        .get(para_idx + 1)
                        .and_then(|np| np.line_segs.first()),
                    para.line_segs.first(),
                ) {
                    let gap_hu = next_first.vertical_pos - cur_first.vertical_pos;
                    if gap_hu > 0 {
                        para_height = crate::renderer::hwpunit_to_px(gap_hu, self.dpi);
                    }
                }
            }
    ...
    fn place_table_fits(
        ...
        if let Some(mp) = measured.get_measured_paragraph(para_idx) {
            ...
            if is_independent_float {
                ...
                //
                // 2026-05-19: wrap=TopAndBottom 비-TAC + vert_offset=0 표는
                // process_section_paragraphs 에서 para_height 를 stored gap 으로 override 했으므로
                // table 누적은 skip (이중 합산 방지).
                let skip_height_for_non_tac_top_bottom = !is_tac_table
                    && matches!(
                        table.common.text_wrap,
                        crate::model::shape::TextWrap::TopAndBottom
                    )
                    && table.common.vertical_offset == 0;
                if !skip_height_for_non_tac_top_bottom {
            ...
            // 2026-05-19: 동일 — wrap=TopAndBottom 비-TAC + vert_offset=0 은 skip.
            let skip_height_for_non_tac_top_bottom = !is_tac_table
                && matches!(
                    table.common.text_wrap,
                    crate::model::shape::TextWrap::TopAndBottom
