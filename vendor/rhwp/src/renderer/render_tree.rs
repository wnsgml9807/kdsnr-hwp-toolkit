    ...
    fn write_json(&self, buf: &mut String) {
        ...
        let (type_str, extra) = match &self.node_type {
            ...
            RenderNodeType::TextRun(tr) => ("TextRun", format!(
                ",\"text\":{},\"pi\":{},\"style\":{{\"ff\":{},\"fs\":{:.2},\"col\":\"#{:06X}\",\"b\":{},\"i\":{},\"u\":{},\"st\":{},\"ls\":{:.2},\"ratio\":{:.3}}}",
                json_escape(&tr.text),
                tr.section_index.map(|_| tr.para_index.unwrap_or(0)).unwrap_or(0),
                json_escape(&tr.style.font_family),
                tr.style.font_size,
                (tr.style.color & 0xFFFFFF),
                tr.style.bold,
                tr.style.italic,
                tr.style.underline as u8,
                tr.style.strikethrough,
                tr.style.letter_spacing,
                tr.style.ratio,
            )),
            ...
            RenderNodeType::Image(im) => ("Image", format!(
                ",\"bin\":{}", im.bin_data_id)),
            RenderNodeType::TextBox => ("TextBox", String::new()),
            RenderNodeType::Equation(_) => ("Equation", String::new()),
            RenderNodeType::Line(ln) => ("Line", format!(
                ",\"x1\":{:.1},\"y1\":{:.1},\"x2\":{:.1},\"y2\":{:.1},\"col\":\"#{:06X}\",\"w\":{:.2}",
                ln.x1, ln.y1, ln.x2, ln.y2, (ln.style.color & 0xFFFFFF), ln.style.width)),
            RenderNodeType::Rectangle(rn) => ("Rect", format!(
                ",\"r\":{:.1},\"fill\":{},\"stroke\":{},\"sw\":{:.2}",
                rn.corner_radius,
                rn.style.fill_color.map(|c| format!("\"#{:06X}\"", c & 0xFFFFFF)).unwrap_or_else(|| "null".into()),
                rn.style.stroke_color.map(|c| format!("\"#{:06X}\"", c & 0xFFFFFF)).unwrap_or_else(|| "null".into()),
                rn.style.stroke_width)),
