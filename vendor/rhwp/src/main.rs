...
fn main() {
    ...
    match args.get(1).map(|s| s.as_str()) {
        ...
        Some("dump-render-tree") => dump_render_tree(&args[2..]),
...
fn print_help() {
    ...
    println!("      --hft-path <폴더>       Hancom HFT 폰트 디렉터리 (한글 음절/자모 path emit)");
    println!();
...
fn load_hft_cache(hft_path: Option<&Path>) -> Option<std::sync::Arc<rhwp::kdsnr_hft::HftCache>> {
    if let Some(dir) = hft_path {
        let mut cache = rhwp::kdsnr_hft::HftCache::new();
        match cache.load_dir(dir) {
            Ok(n) => {
                println!(
                    "HFT 캐시 로드: {}개 글리프 ({}개 패밀리) from {}",
                    n,
                    cache.family_count(),
                    dir.display()
                );
            ...
            Err(e) => {
                eprintln!("경고: HFT 캐시 로드 실패: {}", e);
                None
            }
        }
    } else {
        let arc = rhwp::renderer::kdsnr_hft_global::cache_arc();
        rhwp::renderer::font_runtime_metrics::set_global_hft_cache(arc.clone());
        Some(arc)
    }
}
...
fn export_svg(args: &[String]) {
    ...
    let mut hft_path: Option<std::path::PathBuf> = None;
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--hft-path" => {
                if i + 1 < args.len() {
                    hft_path = Some(std::path::PathBuf::from(&args[i + 1]));
                    i += 2;
                } else {
                    eprintln!("오류: --hft-path 뒤에 디렉토리 경로가 필요합니다.");
                    return;
                }
            }
            _ => {
    ...
    // HFT cache 로드 — 명시 경로가 없으면 embedded cache 를 사용해
    // layout advance 와 SVG/PDF glyph path emit 의 소스를 통일한다.
    let hft_cache_arc = load_hft_cache(hft_path.as_deref());

    // 문서 로드 — paginate 시점에 위 global HFT cache 가 advance 측정에 사용됨.
    ...
    if let Some(arc) = hft_cache_arc {
        doc.set_hft_cache(arc);
    }
...
fn export_pdf(args: &[String]) {
    ...
    if reflow {
        ...
        println!("LineSeg 재계산: {} 문단", reflowed);
    }
...
fn dump_render_tree(args: &[String]) {
    if args.is_empty() {
        eprintln!("사용법: rhwp dump-render-tree <파일.hwp|hwpx> [-p <페이지>] [-o <out.json>] [--hft-path <폴더>]");
        return;
    }
    ...
    for (idx, p) in pages.iter().enumerate() {
        ...
        match doc.get_page_render_tree_native(*p) {
            Ok(json) => {
                buf.push_str("    {\"page\": ");
            ...
            Err(e) => {
                buf.push_str(&format!("    {{\"page\": {}, \"error\": {:?}}}", p, e));
            }
        }
    }
    buf.push_str("\n  ]\n}\n");
    ...
    match out_path {
        Some(p) => {
            ...
            if let Err(e) = fs::write(&p, &buf) {
                eprintln!("쓰기 실패: {} ({})", p, e);
            } else {
                println!("dump-render-tree: {} ({} bytes)", p, buf.len());
            }
        }
        None => print!("{}", buf),
    }
