...
pub const JUNG_SHAPE_TABLE: [u8; 32] = [
    255, 255, 0, 1, 2, 3, 4, 5, 255, 255, 6, 7, 8, 9, 10, 11, 255, 22, 12, 13, 14, 15, 16, 17, 23,
    24, 18, 19, 20, 21, 25, 26,
];
...
pub fn unicode_to_johab(codepoint: u32) -> Option<u16> {
    ...
    Some(0x8000 | ((cho_hwp as u16) << 10) | ((jung_hwp as u16) << 5) | jong_hwp as u16)
}

/// Convert Hwp's Johab-like 16-bit syllable code back to Unicode Hangul.
pub fn johab_to_unicode(code: u16) -> Option<u32> {
    if code & 0x8000 == 0 {
        return None;
    }
    for cp in 0xAC00..=0xD7A3 {
        if unicode_to_johab(cp) == Some(code) {
            return Some(cp);
        }
    }
    None
...
mod tests {
    ...
    fn unicode_to_johab_known() {
        ...
        assert_eq!(unicode_to_johab(0xB123), Some(0x90fd)); // 넣
    }

    #[test]
    fn johab_to_unicode_known() {
        assert_eq!(johab_to_unicode(0x8861), Some(0xAC00)); // 가
        assert_eq!(johab_to_unicode(0x9461), Some(0xB2E4)); // 다
        assert_eq!(johab_to_unicode(0xd065), Some(0xD55C)); // 한
        assert_eq!(johab_to_unicode(0x90fd), Some(0xB123)); // 넣
    }
