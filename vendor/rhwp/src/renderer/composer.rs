...
pub struct ComposedLine {
    ...
    /// 원본 LineSeg.vertical_pos (paragraph 또는 section 기준 line top y, HWPUNIT).
    /// raid 24 RE 결과 활용: HWPX 가 저장한 한컴 layout 결과를 직접 적용해
    /// rhwp 의 누적-계산 결함 (inline TAC table 같은 경우 line height 가 잘못 잡혀
    /// 다음 paragraph 가 박스 안 침투하는 버그) 을 회피한다.
...
/// G-W-3a wire: env-gate (`RHWP_USE_KDSNR_LAYOUT=1`) 으로 reflow_line_segs 강제 호출.
///
/// 기존 `compose_paragraph(para)` 는 hwpx 의 stored `<hp:lineseg>` 를 그대로 trust
/// 하지만, env-on 시 paragraph 의 clone 만들고 `reflow_line_segs` 호출하여 byte-eq
/// line break 위치 재계산. 그 후 compose_paragraph 호출.
///
/// avail_width 는 paragraph 자체의 stored `line_segs[0].segment_width` 에서 자동
/// 추출 (HWPUNIT → px 변환). stored 가 없으면 reflow 안 호출.
///
/// reflow_line_segs 안에는 ppt_compose_break (한컴 byte-eq) PoC 가 들어 있어
/// env-on 시 자동으로 fire.
#[cfg(not(target_arch = "wasm32"))]
pub fn compose_paragraph_with_reflow(
    para: &Paragraph,
    ...
    if std::env::var("RHWP_USE_KDSNR_LAYOUT").is_ok() {
        let avail_hwp = para
            ...
            .unwrap_or(0);
        if avail_hwp > 0 {
            let avail_px = crate::renderer::hwpunit_to_px(avail_hwp, dpi);
            let mut cloned = para.clone();
            reflow_line_segs(&mut cloned, avail_px, styles, dpi);
            return compose_paragraph(&cloned);
        }
    }
...
pub fn compose_paragraph_with_reflow(
    para: &Paragraph,
    _styles: &crate::renderer::style_resolver::ResolvedStyleSet,
    _dpi: f64,
) -> ComposedParagraph {
    compose_paragraph(para)
}

/// G-W-3a: compose_section wrapper. paragraph 별로 `compose_paragraph_with_reflow` 호출.
pub fn compose_section_with_reflow(
    ...
    section
        .paragraphs
        .iter()
        .map(|p| compose_paragraph_with_reflow(p, styles, dpi))
        .collect()
}
        ...
        if let (true, Some(nl_pos)) = (has_tac, newline_pos) {
            ...
            if !pre_text.is_empty() && !lines.is_empty() {
                ...
                lines.push(ComposedLine {
                    ...
                    vertical_pos: line_seg.vertical_pos,
                });
            }
...
/// `<hp:linesegarray>` 가 실제 해상된 layout 값을 담고 있는지 판정.
///
/// 한 컴 12+ 의 HWPX 출력은 본문/셀 가리지 않고 모든 paragraph 의 lineseg 를
/// 실제 wrap 결과로 기록한다 (horzsize = 실제 cell content 폭, vertsize = 실제
/// 줄 높이). 우리 toolkit 의 `lineseg_gen.py` 도 동일 계약. 이 경우 render-time
/// 재계산 (`reflow_line_segs`) 은 손실 — 한 컴 의 정밀 metric 을 rhwp 의
/// 폰트-크기 기반 근사로 덮어씌우기 때문.
///
/// HWP 바이너리 경로의 셀 paragraph 는 lineseg 가 placeholder (단일 seg,
/// horzsize=0) 라 `segment_width <= 0` 이 거의 보장. 그 경우만 reflow 로 fallback.
pub(crate) fn has_resolved_linesegs(p: &crate::model::paragraph::Paragraph) -> bool {
    p.line_segs
        .first()
        .map(|ls| ls.line_height > 0 && ls.segment_width > 0)
        .unwrap_or(false)
}
