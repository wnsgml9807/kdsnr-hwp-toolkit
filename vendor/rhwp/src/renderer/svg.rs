...
pub struct SvgRenderer {
    ...
    /// HFT 글리프 캐시. Some 이면 캐시 hit 시 <text> 대신 <path> emit.
    /// `--hft-path <dir>` CLI flag 로 활성화. 자세한 내용은
    /// kdsnr-hwp-toolkit/work/hft_re/INTEGRATION_CONTEXT.md 섹션 18.
    #[cfg(not(target_arch = "wasm32"))]
    pub hft_cache: Option<std::sync::Arc<kdsnr_hft::HftCache>>,
}
...
impl SvgRenderer {
    ...
    /// HFT 글리프 캐시 설정. cache 활성 상태에서 `draw_text` 가 받은 char 가
    /// 캐시에 있으면 `<text>` 대신 `<path>` 를 emit. miss 면 기존 폴백.
    ///
    /// Global font_runtime_metrics cache 도 함께 set 해 layout 단계의
    /// `measure_char_advance_em` 가 동일한 HFT advance 를 반환하게 한다.
    /// 이게 없으면 layout 은 TTF advance, emit 은 HFT advance 가 되어
    /// 줄 내 누적 오차가 발생한다 (raid 23 v2 → v3 fix).
    #[cfg(not(target_arch = "wasm32"))]
    pub fn set_hft_cache(&mut self, cache: std::sync::Arc<kdsnr_hft::HftCache>) {
        super::font_runtime_metrics::set_global_hft_cache(cache.clone());
        self.hft_cache = Some(cache);
    }

    /// `text` 의 모든 char 가 HFT 캐시에 있으면 SVG `<g><path>` 시퀀스를 생성.
    /// 한 char 라도 miss 면 `None` 을 반환해 호출자가 기존 `<text>` 폴백을 쓰게 한다.
    ///
    /// 좌표 변환은 macOS native consumer `FUN_00091c30` 와 동일:
    ///   translate(baseline_x, baseline_y) → scale(s, -s)
    /// 여기서 `s = font_size / em` (em-units → px). y-flip 으로 TrueType-style
    /// y-up 좌표를 화면 y-down 으로 변환.
    ///
    /// 부분 hit (한글+영문 mixed) 처리: PoC 단계라 전부 hit 일 때만 swap 한다.
    /// mixed run 의 한글만 path 로 emit 하려면 advance 측정 차이 (HFT em advance
    /// vs rhwp 폰트 metric DB) 가 누적돼 위치가 어긋날 수 있어 추가 작업 필요.
    #[cfg(not(target_arch = "wasm32"))]
    fn try_emit_hft_paths(
        text: &str,
        x: f64,
        y: f64,
        style: &TextStyle,
        ...
        // HFT cache lookup 은 raw_font_family (substitute 전 원본 HWP HFT face
        // 이름) 우선. 빈 문자열이면 substituted font_family 로 폴백.
        let raw_primary =
            super::style_resolver::primary_font_name(&style.raw_font_family).to_string();
        let sub_primary = super::style_resolver::primary_font_name(&style.font_family).to_string();

        // 한 char 라도 HFT 캐시에 있어야 hook 활성화 (mixed run 도 일부만 swap).
        // 그렇지 않으면 영문/숫자 전용 run 까지 path emit 시도 (전부 miss 라 의미 없음).
        //
        // cache.get 은 내부에서 hftinfo.dat alias map 까지 시도하므로 별도
        // blanket fallback (예: 무조건 HCHGGGT) 을 두지 않는다. alias 가 없는
        // face (예: 한컴바탕 / HCR Batang TTF 류) 는 자연스럽게 None 으로
        // 떨어져 TTF text emit 로 흘러간다.
        // 2026-05-19: HFT outline 차단 제거. 이전 차단 사유 "GT 가 HCR Batang TTF 로
        // substitute" 는 부분적 추론이고, hwpx 사양 (raw HFT face name) 의 진짜 glyph
        // 는 한컴 HFT 바이너리 데이터. GT 픽셀도 같은 HFT 소스에서 파생. 따라서
        // HFT vector path emit 이 hwpx + GT 둘 다 가까워지는 정공법.
        // (이전 pixel match 30.2→28.8% 감소는 face shape 가 아니라 advance/raster
        //  artifact 일 가능성. 사용자 시각 "셀 폰트 다름" 회복 우선.)
        let mut any_hit = false;
        let mut glyphs: Vec<(char, Option<kdsnr_hft::Glyph>)> =
            Vec::with_capacity(text.chars().count());
        let dump_face = std::env::var("RHWP_FACE_DUMP").is_ok();
        for ch in text.chars() {
            let code = ch as u32;
            let mut hit_via = "";
            let g = if !raw_primary.is_empty() {
                if let Some(g) = cache.get(&raw_primary, code).cloned() {
                    hit_via = "raw";
                    Some(g)
                } else if let Some(g) = cache.get(&sub_primary, code).cloned() {
                    hit_via = "sub";
                    Some(g)
                } else {
                    None
                ...
                cache.get(&sub_primary, code).cloned().map(|g| {
                    hit_via = "sub";
                    g
                })
            };
            if let Some(ref gg) = g {
                any_hit = true;
                if dump_face && (0xAC00..=0xD7A3).contains(&code) {
                    eprintln!(
                        "FACE ch='{}' raw={:?} sub={:?} hit_via={} em={} advance={}",
                        ch, raw_primary, sub_primary, hit_via, gg.em, gg.advance
                    );
                }
            }
            glyphs.push((ch, g));
        ...
        // 좌표: rhwp 의 compute_char_positions 가 계산한 layout 위치 그대로 사용한다.
        // 자체 cur_x 누적 X → layout-emit 100% sync, 줄 내 누적 오차 없음.
        let char_positions = super::layout::compute_char_positions(text, style);
        ...
        // style.ratio (HWP "글자 너비", e.g. 0.95) 는 X 축에만 적용된다.
        // 누락 시 advance 는 ratio 적용 후 좁아진 위치에 다음 글자가 놓이지만
        // HFT path 는 원본 1.0 폭으로 그려져 우측이 다음 글자와 겹친다 (값/은
        // 등 polysyllable cluster 가 뭉개지는 원인). TTF text emit 의
        // `scale({ratio},1)` 과 대칭이 되도록 X scale 에 ratio 를 곱한다.
        let ratio = if style.ratio > 0.0 { style.ratio } else { 1.0 };
        for (i, (ch, g_opt)) in glyphs.iter().enumerate() {
            let cur_x = x + char_positions.get(i).copied().unwrap_or(0.0);
            match g_opt {
                Some(g) => {
                    let base_s = font_size / (g.em as f64).max(1.0);
                    let sx = base_s * ratio;
                    let sy = base_s;
                    out.push_str(&format!(
                        "<g transform=\"translate({:.4} {:.4}) scale({:.6} -{:.6})\">\
                         <path d=\"{}\" fill=\"{}\" fill-rule=\"nonzero\"/></g>\n",
                        cur_x, y, sx, sy, g.d, color
                    ));
                }
                None => {
                    // miss char → 단일 <text>. 위치는 layout 의 char_positions[i].
                    let escaped = escape_xml(&ch.to_string());
                    out.push_str(&format!(
                        "<text x=\"{:.4}\" y=\"{:.4}\" font-family=\"{}\" font-size=\"{}\" fill=\"{}\">{}</text>\n",
                        cur_x, y, escape_xml(&font_family), font_size, color, escaped
                    ));
                }
            }
        }
    ...
    fn render_node(&mut self, node: &RenderNode) {
        ...
        match &node.node_type {
            ...
            RenderNodeType::TableCell(ref tc) if tc.clip => {
                ...
                // 셀 콘텐츠가 bbox 끝까지 채울 때 stroke 의 반쪽 (0.5 px 정도) 이 경계 밖으로
                // 새어 잘리는 케이스가 있다 (예: 인라인 수식 안의 brace path stroke). 콘텐츠
                // 바운딩 박스보다 stroke 두께만큼 여유를 둬서 클립한다 — 시각적 경계 정렬은
                // 동일하지만 외곽 stroke 가 보존된다.
                const STROKE_MARGIN: f64 = 1.0;
                self.defs.push(format!(
                    "<clipPath id=\"{}\"><rect x=\"{}\" y=\"{}\" width=\"{}\" height=\"{}\"/></clipPath>\n",
                    clip_id,
                    node.bbox.x - STROKE_MARGIN,
                    node.bbox.y - STROKE_MARGIN,
                    node.bbox.width + STROKE_MARGIN * 2.0,
                    node.bbox.height + STROKE_MARGIN * 2.0,
                ));
                self.output
                    .push_str(&format!("<g clip-path=\"url(#{})\">", clip_id));
            }
...
impl Renderer for SvgRenderer {
    ...
    fn draw_text(&mut self, text: &str, x: f64, y: f64, style: &TextStyle) {
        ...
        let text = &text
            .chars()
            .map(|ch| crate::renderer::layout::map_pua_bullet_char(ch))
            .collect::<String>();

        // HFT cache hook (raid 23): 캐시에 모든 char 가 있으면 <text> 대신
        // <path> 를 emit 한다. 단 글자가 mixed (한글+영문 등) 라 일부만 hit
        // 이면 폴백한다. PoC 단계.
        #[cfg(not(target_arch = "wasm32"))]
        {
            if let Some(cache) = self.hft_cache.clone() {
                if let Some(svg) = Self::try_emit_hft_paths(text, x, y, style, &cache) {
                    self.output.push_str(&svg);
                    ...
                    if let Some(set) = self.font_codepoints.get_mut(&style.font_family) {
                        for ch in text.chars() {
                            set.insert(ch);
                        }
                    }
        ...
        if style.shadow_type > 0 {
            ...
            for (cluster_idx, (char_idx, cluster_str)) in clusters.iter().enumerate() {
                ...
                let char_y = y + dy;
                // 2026-05-18: scale(ratio,1) horizontal squeeze 제거.
                // char_positions 에 ratio 가 이미 곱해져 advance 가 narrow 이고,
                // glyph 자체는 GT (한컴 native PDF) 와 동일하게 wide 로 emit.
                // 사용자 시각 비교 결과 GT 본문 글자가 wide glyph + narrow advance.
                self.output.push_str(&format!(
                    "<text x=\"{}\" y=\"{}\" {}>{}</text>\n",
                    char_x,
                    char_y,
                ...
                let _ = has_ratio; // suppress unused warning in shadow branch
            }
        }
        ...
        for (cluster_idx, (char_idx, cluster_str)) in clusters.iter().enumerate() {
            ...
            // 2026-05-18: scale(ratio,1) horizontal squeeze 제거.
            // char_positions 가 이미 ratio 적용된 narrow advance 를 갖고 있어
            // glyph 자체는 1.0 폭으로 emit 해야 GT 와 시각상 일치.
            self.output.push_str(&format!(
                "<text x=\"{}\" y=\"{}\" {}>{}</text>\n",
                char_x,
                y,
            ...
            let _ = has_ratio; // suppress unused warning
        }
...
fn known_font_filenames(font_name: &str) -> Vec<&'static str> {
    match font_name {
        // toolkit assets/fonts (Hancom official HCR Batang/Dotum 2017 release)
        "HCR Batang" => vec!["HCRBatang.ttf", "HCRBATANG.TTF", "HCR Batang.ttf"],
        "HCR Batang Bold" => vec![
            "HCRBatang-Bold.ttf",
            "HCRBatangBold.ttf",
            "HCRBATANG-BOLD.TTF",
        ],
        "HCR Dotum" => vec!["HCRDotum.ttf", "HCRDOTUM.TTF", "HCR Dotum.ttf"],
        "HCR Dotum Bold" => vec!["HCRDotum-Bold.ttf", "HCRDotumBold.ttf", "HCRDOTUM-BOLD.TTF"],
        // Hancom Office TTF Install dir (HBatang.TTF family-name="Haansoft Batang" + 신명/한양 series)
        // 2026-05-18: HCRBatang.ttf 폴백 제거 — assets/HCRBatang.ttf 가 extra_paths 첫번째라
        // HBatang.TTF 가 아닌 HCRBatang.ttf 가 매칭되어 SVG @font-face data 가 잘못된 face
        // (HCR, 얇은 stroke) 를 임베드. find_font_file 은 정확한 face 매칭 외엔 fallback 금지.
        "Haansoft Batang" | "HaansoftBatang" | "Haansoft-Batang" => {
            vec!["HBatang.TTF", "HBATANG.TTF", "HBatang.ttf"]
        }
        "Haansoft Dotum" | "HaansoftDotum" | "Haansoft Gothic" => {
            vec!["HDotum.TTF", "HDOTUM.TTF"]
        }
        // hamcho-r 폴백 (legacy)
        "함초롬바탕" | "함초롱바탕" | "한컴바탕" => {
            vec!["HCRBatang.ttf", "hamchob-r.ttf", "HBATANG.TTF"]
        }
        "함초롬돋움" | "함초롱돋움" | "한컴돋움" => {
            vec!["HCRDotum.ttf", "hamchod-r.ttf", "HDOTUM.TTF"]
        }
        ...
        "바탕" | "Batang" => vec!["HCRBatang.ttf", "batang.ttc", "BATANG.TTC", "hamchob-r.ttf"],
        "돋움" | "Dotum" => vec!["HCRDotum.ttf", "dotum.ttc", "DOTUM.TTC", "hamchod-r.ttf"],
        ...
        "바탕체" | "BatangChe" => vec!["HCRBatang.ttf", "batang.ttc", "hamchob-r.ttf"],
        "휴먼명조" => vec!["HYMJRE.TTF", "hamchob-r.ttf"],
        "새바탕" | "새돋움" | "새굴림" | "새궁서" => {
            vec!["HCRBatang.ttf", "HCRDotum.ttf", "hamchob-r.ttf"]
        }
        _ => vec![],
    }
}
...
pub fn generate_font_style(renderer: &SvgRenderer, font_paths: &[std::path::PathBuf]) -> String {
    ...
    if std::env::var("RHWP_EMBED_DUMP").is_ok() {
        eprintln!("[font-embed] codepoints map keys ({}):", codepoints.len());
        for (k, set) in codepoints.iter() {
            eprintln!("  key={:?} (chars={})", k, set.len());
        }
    }
                    ...
                    if let Ok(font_data) = std::fs::read(&font_path) {
                        ...
                        match subsetter::subset(&font_data, 0, &remapper) {
                            Ok(subset_data) => {
                                ...
                                // Sniff format from the path/data: .ttf → truetype, .otf → opentype
                                let is_otf = font_path
                                    .extension()
                                    .and_then(|e| e.to_str())
                                    .map(|e| e.eq_ignore_ascii_case("otf"))
                                    .unwrap_or(false);
                                let (mime, fmt) = if is_otf {
                                    ("font/otf", "opentype")
                                } else {
                                    ("font/ttf", "truetype")
                                };
