...
fn parse_font(e: &quick_xml::events::BytesStart, doc_info: &mut DocInfo, font_group: usize) {
    ...
    let mut alt_type: u8 = 0;

    for attr in e.attributes().flatten() {
        match attr.key.as_ref() {
            b"face" => name = attr_str(&attr),
            b"type" => {
                // HWPX font @type → 내부 alt_type. HFT → 한양/안상수 등 한컴 전용 폰트
                // (시스템 미설치) 매핑이 작동하려면 반드시 2 로 표기되어야 한다.
                let s = attr_str(&attr);
                alt_type = match s.as_str() {
                    "TTF" => 1,
                    "HFT" => 2,
                    _ => 0,
                };
...
fn parse_char_shape(
    ...
    for attr in e.attributes().flatten() {
        match attr.key.as_ref() {
            ...
            b"borderFillIDRef" => cs.border_fill_id = parse_border_fill_ref(&attr),
            _ => {}
        }
    }
...
fn parse_para_shape(
    ...
    for attr in e.attributes().flatten() {
        match attr.key.as_ref() {
            ...
            b"condense" => ps.condense = parse_u8(&attr),
            _ => {}
        }
    }
                    ...
                    match local {
                        b"align" => {
                            for attr in ce.attributes().flatten() {
                                match attr.key.as_ref() {
                                    ...
                                    b"vertical" => {
                                        let val = attr_str(&attr);
                                        ps.vertical_align = match val.as_str() {
                                            "BASELINE" => 0,
                                            "WORD" => 1,
                                            "TOP" => 2,
                                            "CENTER" => 3,
                        ...
                        b"border" => {
                            for attr in ce.attributes().flatten() {
                                match attr.key.as_ref() {
                                    b"borderFillIDRef" => {
                                        ps.border_fill_id = parse_border_fill_ref(&attr)
                                    }
                                    ...
                                    // Connect this paragraph's border with adjacent
                                    // same-borderFill paragraphs into one region
                                    // (attr1 bit 28); ignoreMargin is bit 29.
                                    b"connect" => {
                                        if parse_bool(&attr) {
                                            ps.attr1 |= 1 << 28;
                                        }
                    ...
                    match local {
                        ...
                        b"diagonal" => {
                            for attr in ce.attributes().flatten() {
                                match attr.key.as_ref() {
                                    b"type" => {
                                        bf.diagonal.diagonal_type = parse_border_line_code(&attr)
                                    }
                        ...
                        b"slash" => {
                            for attr in ce.attributes().flatten() {
                                match attr.key.as_ref() {
                                    b"type" => {
                                        if parse_border_line_code(&attr) != 0 {
                                            bf.attr = (bf.attr & !(0x07 << 2)) | (0b010 << 2);
                                        }
                                    }
                                    b"width" => bf.diagonal.width = parse_border_width(&attr),
                                    b"color" => bf.diagonal.color = parse_color(&attr),
                        ...
                        b"backSlash" => {
                            for attr in ce.attributes().flatten() {
                                match attr.key.as_ref() {
                                    b"type" => {
                                        if parse_border_line_code(&attr) != 0 {
                                            bf.attr = (bf.attr & !(0x07 << 5)) | (0b010 << 5);
                                        }
                                    }
                                    b"width" => bf.diagonal.width = parse_border_width(&attr),
                                    b"color" => bf.diagonal.color = parse_color(&attr),
...
fn parse_numbering(
    ...
    if !is_empty_event(e) {
        let mut buf = Vec::new();
        // `<hh:paraHead ... numFormat="..">^1.</hh:paraHead>`: the format template
        // is the element text, so hold the attr-parsed head pending until its text.
        let mut pending: Option<(usize, NumberingHead)> = None;
        let mut pending_fmt = String::new();
        loop {
            match reader.read_event_into(&mut buf) {
                Ok(Event::Empty(ref ce)) if local_name(ce.name().as_ref()) == b"paraHead" => {
                    let (level, head, fmt) = parse_para_head(ce, &mut num);
                    if level > 0 && level <= 7 {
                        num.heads[level - 1] = head;
                        num.level_formats[level - 1] = fmt;
                    }
                }
                Ok(Event::Start(ref ce)) if local_name(ce.name().as_ref()) == b"paraHead" => {
                    let (level, head, fmt) = parse_para_head(ce, &mut num);
                    pending = Some((level, head));
                    pending_fmt = fmt; // attribute fallback; element text appends below
                }
                Ok(Event::Text(ref t)) => {
                    if pending.is_some() {
                        if let Ok(s) = t.decode() {
                            pending_fmt.push_str(&s);
                        }
                    }
                }
                Ok(Event::End(ref ee)) => {
                    ...
                    if name == b"paraHead" {
                        if let Some((level, head)) = pending.take() {
                            if level > 0 && level <= 7 {
                                num.heads[level - 1] = head;
                                num.level_formats[level - 1] = pending_fmt.trim().to_string();
                            }
                            pending_fmt.clear();
                        }
                    } else if name == b"numbering" {
                        break;
                    }
                }
                Ok(Event::Eof) => break,
...
/// hwpx `numFormat` string -> HWP number-format code (spec table 43).
fn num_format_code(s: &str) -> u8 {
    match s {
        "CIRCLED_DIGIT" => 1,
        "ROMAN_CAPITAL" => 2,
        "ROMAN_SMALL" => 3,
        ...
        _ => 0, // DIGIT (and unknown)
    }
}

/// Parse a `<hh:paraHead>` start/empty tag's attributes; the format template is
/// the element text (captured by the caller). Records the level start number.
fn parse_para_head(
    ce: &quick_xml::events::BytesStart,
    num: &mut Numbering,
) -> (usize, NumberingHead, String) {
    let mut level = 0usize;
    let mut head = NumberingHead::default();
    let mut fmt = String::new();
    let mut start_num: Option<u32> = None;
    for attr in ce.attributes().flatten() {
        match attr.key.as_ref() {
            b"level" => level = parse_u32(&attr) as usize,
            b"start" => start_num = Some(parse_u32(&attr)),
            b"text" => fmt = attr_str(&attr),
            b"numFormat" => head.number_format = num_format_code(&attr_str(&attr)),
...
fn parse_border_line_code(attr: &quick_xml::events::attributes::Attribute) -> u8 {
    match attr_str(attr).as_str() {
        "NONE" => 0,
        "SOLID" => 1,
        "DASH" => 2,
...
fn parse_border_width(attr: &quick_xml::events::attributes::Attribute) -> u8 {
    let s = attr_str(attr);
    // Reverse of serializer::hwpx::header::border_width_mm().
    let mm: f64 = s
        .split_whitespace()
        .next()
