...
pub struct Equation {
    ...
    /// 라운드트립/RE용 원본 HWPTAG_EQEDIT 레코드 데이터
    pub raw_eqedit_data: Vec<u8>,
    /// 수식에 붙은 HWPTAG_SHAPE_COMPONENT 원본 폭/높이.
    pub shape_original_width: u32,
    pub shape_original_height: u32,
    /// 수식에 붙은 HWPTAG_SHAPE_COMPONENT 현재 폭/높이.
    pub shape_current_width: u32,
    pub shape_current_height: u32,
}
...
pub struct Field {
    ...
    pub memo_index: u32,
    /// MEMO 필드가 참조하는 숨은 메모 본문 문단들.
    ///
    /// HWP 바이너리는 MEMO 필드 앵커와 실제 메모 본문을 분리해서 저장한다.
    /// HWPX 변환에서는 fieldBegin 내부 subList로 다시 합쳐야 하므로 파서에서
    /// 필드 순서/메모 번호를 기준으로 연결한다.
