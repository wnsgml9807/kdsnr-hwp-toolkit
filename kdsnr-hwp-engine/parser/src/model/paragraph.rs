...
/// 문단 내부의 원본 흐름 순서.
///
/// 기존 모델은 `text`와 `controls`를 따로 보관한다. 렌더링에는 충분하지만,
/// 문항 분할처럼 "글자, 표, 그림이 한 문단 안에서 어떤 순서로 섞였는가"가
/// 중요한 작업에는 부족하다. 이 스트림은 parser가 본 원래 순서를 보존한다.
...
pub struct Paragraph {
    ...
    pub text: String,
    /// 원본 paragraph item 순서 (텍스트 조각 + controls[] index)
...
impl Paragraph {
    ...
    pub fn split_at(&mut self, char_offset: usize) -> Paragraph {
        ...
        self.items = if self.text.is_empty() {
            self.controls
                .iter()
                .enumerate()
            ...
            if !self.text.is_empty() {
                items.push(ParagraphItem::Text(self.text.clone()));
            }
            items.extend(
                self.controls
                    .iter()
        ...
        Paragraph {
            text: new_text.clone(),
            items: if new_text.is_empty() {
                Vec::new()
            } else {
                vec![ParagraphItem::Text(new_text.clone())]
            },
