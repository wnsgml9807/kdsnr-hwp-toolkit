...
impl DocumentCore {
    pub fn from_bytes(data: &[u8]) -> Result<DocumentCore, HwpError> {
        ...
        // G-W-3a wire: env-gate RHWP_USE_KDSNR_LAYOUT 시 paragraph 별 reflow_line_segs
        // 강제 호출 → 한컴 byte-eq line break positions 적용. 그 외 stored line_segs 그대로.
        let composed = document
            .sections
            .iter()
            .map(|s| {
                crate::renderer::composer::compose_section_with_reflow(s, &styles, DEFAULT_DPI)
            })
            .collect();
    ...
    /// 셀 단락의 placeholder reflow (parse 시점 자동 보정 전용).
    ///
    /// 한컴 HWPX 셀 단락의 lineseg.horzsize 가 cell_inner_width 와 동치 (self-encoded).
    /// 그 폭을 그대로 wrap 에 사용한다. segment_width 가 0 이면 fallback_col_width_px.
    /// 검출: `needs_line_seg_reflow || needs_reflow_broadly` (line_height>0 placeholder
    /// 도 잡기 위해). 셀 내부 표는 재귀.
    fn reflow_zero_height_table_cells_inner(
        table: &mut crate::model::table::Table,
        fallback_col_width_px: f64,
        styles: &ResolvedStyleSet,
        dpi: f64,
    ) {
        for cell in &mut table.cells {
            for cp in &mut cell.paragraphs {
                if Self::needs_line_seg_reflow(cp) || Self::needs_reflow_broadly(cp) {
                    let avail_px = cp
                        .line_segs
                        .first()
                        .map(|s| s.segment_width)
                        .filter(|w| *w > 0)
                        .map(|w| crate::renderer::hwpunit_to_px(w, dpi))
                        .unwrap_or(fallback_col_width_px);
                    reflow_line_segs(cp, avail_px, styles, dpi);
                }
                for inner_ctrl in &mut cp.controls {
                    if let Control::Table(ref mut inner_table) = inner_ctrl {
                        Self::reflow_zero_height_table_cells_inner(
                            inner_table,
    ...
    fn reflow_zero_height_paragraphs(document: &mut Document, styles: &ResolvedStyleSet, dpi: f64) {
        ...
        for section in &mut document.sections {
            ...
            for para in &mut section.paragraphs {
                // 본문 문단 reflow — 한컴 .hwp/.hwpx 는 long-text paragraph 의 line_segs
                // 를 placeholder 1 개만 cache 하고 viewer 가 매번 reflow 한다. 우리도
                // 동일하게 본문에 needs_reflow_broadly 까지 적용해야 cell 처리와
                // 일관되고, "len==1 + long text + no \n" 케이스가 1 줄로 cram 되는
                // 결함을 피할 수 있다.
                ...
                // 표 셀 내부 문단 reflow (parse 시점 자동 보정).
                //
                // 한컴 HWPX 는 셀 단락의 line_segs 를 placeholder 1 개로 저장하며
                // 그 horzsize 속성에 실제 cell_inner_width 를 self-encode 한다.
                // 따라서 segment_width 가 0 보다 크면 그것을 wrap 폭으로 직접 사용,
                // 미설정 시에만 col_width 로 fallback. 검출 조건은 본문과 동일하게
                // needs_reflow_broadly 까지 확장하여 line_height>0 인 placeholder
                // 도 잡는다. 셀 내부 표는 재귀 처리.
                for ctrl in &mut para.controls {
                    if let Control::Table(ref mut table) = ctrl {
                        Self::reflow_zero_height_table_cells_inner(table, col_width, styles, dpi);
                    }
                }
            }
    ...
    fn needs_reflow_broadly(para: &crate::model::paragraph::Paragraph) -> bool {
        ...
        // 한컴 placeholder multi-line 의도 검출 — cell paragraph 에서 한컴이 lineseg
        // 1 개만 cache 하면서 line_height 에는 multi-line 의 누적 높이 (또는 의도된
        // wrap 높이) 를 저장해 두는 패턴. 예: sci Q20 보기 셀 ㄷ 항목 36 chars
        // (LONG_TEXT_THRESHOLD 미만이지만) lh=2580 vs th=1150 (2.24×) → 명확히
        // multi-line 의도. text 가 있고 lh > th × 1.5 이면 reflow 후보.
        if para.line_segs.len() == 1 && !para.text.is_empty() && !para.text.contains('\n') {
            let seg = &para.line_segs[0];
            if seg.text_height > 0
                && seg.line_height > 0
                && (seg.line_height as i64) * 2 > (seg.text_height as i64) * 3
            {
                return true;
            }
        }
