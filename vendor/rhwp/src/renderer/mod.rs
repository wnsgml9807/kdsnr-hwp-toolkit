...
pub mod font_metrics_hancom;
#[cfg(not(target_arch = "wasm32"))]
pub mod font_runtime_metrics;
pub mod height_measurer;
pub mod html;
#[cfg(not(target_arch = "wasm32"))]
pub mod kdsnr_bridge;
pub mod kdsnr_hft_global;
...
pub struct TextStyle {
    /// 글꼴 이름 (substitute 적용된 TTF-친화 이름, e.g. "Haansoft Batang")
    pub font_family: String,
    /// substitute 전 HWP 원본 face 이름 (e.g. "신명 중명조", "한양신명조").
    /// HFT cache lookup 에 사용. 빈 문자열이면 font_family 폴백.
    ...
    /// 활성 lang_index 외 다른 언어 슬롯이 견체 face 를 사용 중인지 여부.
    /// 예: charPr.fontRef hangul=함초롬바탕(regular) + latin=한양견명조(heavy) — 한국어
    /// 전용 lang_index 로 resolved 된 경우에도 Latin 부분은 heavy 로 그려져야 한다.
    /// 정확한 per-character lang switching 이 미구현인 동안의 근사 보정.
    pub any_lang_heavy_face: bool,
}
