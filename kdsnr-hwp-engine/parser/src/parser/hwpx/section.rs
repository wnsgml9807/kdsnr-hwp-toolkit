...
use super::utils::{
    attr_str, local_name, parse_bool, parse_border_fill_ref, parse_color, parse_i16, parse_i32,
...
fn parse_page_border_fill_attrs(e: &quick_xml::events::BytesStart, fill: &mut PageBorderFill) {
    for attr in e.attributes().flatten() {
        if attr.key.as_ref() == b"borderFillIDRef" {
            fill.border_fill_id = parse_border_fill_ref(&attr);
        }
    }
}

fn parse_page_border_offset(e: &quick_xml::events::BytesStart, fill: &mut PageBorderFill) {
    for attr in e.attributes().flatten() {
        match attr.key.as_ref() {
            b"left" => fill.spacing_left = parse_i16(&attr),
            b"right" => fill.spacing_right = parse_i16(&attr),
            b"top" => fill.spacing_top = parse_i16(&attr),
            b"bottom" => fill.spacing_bottom = parse_i16(&attr),
            _ => {}
        }
    }
}
...
fn parse_paragraph(
    ...
    let mut item_parts: Vec<ParagraphItem> = Vec::new();
            ...
            Ok(Event::Start(ref ce)) => {
                ...
                match local {
                    b"run" => {
                        ...
                        let changed = char_shape_changes
                            .last()
                            .map(|(_, id)| *id != current_char_shape_id)
                            .unwrap_or(true);
                        if changed {
                    ...
                    b"t" => {
                        ...
                        push_text_part(&mut text_parts, &mut item_parts, text);
                    ...
                    b"tbl" => {
                        ...
                        item_parts.push(ParagraphItem::Control(para.controls.len()));
                    ...
                    b"secPr" => {
                        ...
                        para.controls
                            .push(Control::SectionDef(Box::new(sd.clone())));
                        text_parts.push("\u{0002}".to_string());
                        item_parts.push(ParagraphItem::Control(para.controls.len() - 1));
                        sec_def = Some(sd);
            ...
            Ok(Event::Empty(ref ce)) => {
                ...
                match local {
                    b"lineBreak" | b"softHyphen" => {
                        push_text_part(&mut text_parts, &mut item_parts, "\n".to_string());
                    }
                    b"columnBreak" => {
                        push_text_part(&mut text_parts, &mut item_parts, "\n".to_string());
                    }
    ...
    para.items = coalesce_paragraph_items(item_parts);
...
fn push_text_part(text_parts: &mut Vec<String>, item_parts: &mut Vec<ParagraphItem>, text: String) {
    if text.is_empty() {
        return;
    }
    text_parts.push(text.clone());
    match item_parts.last_mut() {
        Some(ParagraphItem::Text(prev)) => prev.push_str(&text),
        _ => item_parts.push(ParagraphItem::Text(text)),
    }
}

fn coalesce_paragraph_items(items: Vec<ParagraphItem>) -> Vec<ParagraphItem> {
    ...
    loop {
        match reader.read_event_into(&mut buf) {
            ...
            Ok(Event::GeneralRef(ref r)) => {
                // quick-xml 0.39+ 가 `&lt;` `&gt;` `&amp;` `&apos;` `&quot;`
                // `&#NN;` 같은 entity reference 를 별도 이벤트로 분리.
                // resolve 해서 텍스트에 포함시킨다 (이전 버전은 Text 안에
                // 통합되어 있어 자동 처리됐으나 0.39 부터 명시 필요).
                let raw = r.decode().unwrap_or_default();
                let resolved: Option<&'static str> = match raw.as_ref() {
                    "lt" => Some("<"),
                    "gt" => Some(">"),
                    "amp" => Some("&"),
                    "apos" => Some("'"),
...
fn parse_table(
    ...
    for attr in e.attributes().flatten() {
        match attr.key.as_ref() {
            ...
            b"borderFillIDRef" => table.border_fill_id = parse_border_fill_ref(&attr),
            b"pageBreak" => {
                let val = attr_str(&attr);
                table.page_break = match val.as_str() {
                    "CELL" | "CELL_BREAK" => TablePageBreak::CellBreak,
                    "ROW" | "ROW_BREAK" | "TABLE" => TablePageBreak::RowBreak,
        ...
        match reader.read_event_into(&mut buf) {
            ...
            Ok(Event::Empty(ref ce)) => {
                ...
                match local {
                    ...
                    b"cellzone" => {
                        ...
                        for attr in ce.attributes().flatten() {
                            match attr.key.as_ref() {
                                ...
                                b"borderFillIDRef" => {
                                    zone.border_fill_id = parse_border_fill_ref(&attr)
                                }
                                _ => {}
                            }
                        }
                    ...
                    b"label" => {
                        table.label = Some(parse_table_label(ce));
                    }
                    _ => {}
                }
            }
    ...
    table.common.attr = synthesize_common_obj_attr(&table.common);
...
fn parse_table_label(e: &quick_xml::events::BytesStart) -> crate::model::table::TableLabel {
    let mut label = crate::model::table::TableLabel::default();
    for attr in e.attributes().flatten() {
        match attr.key.as_ref() {
            b"topmargin" => label.top_margin = parse_i32(&attr),
            b"leftmargin" => label.left_margin = parse_i32(&attr),
            b"boxwidth" => label.box_width = parse_i32(&attr),
            b"boxlength" => label.box_length = parse_i32(&attr),
            b"boxmarginhor" => label.box_margin_hor = parse_i32(&attr),
            b"boxmarginver" => label.box_margin_ver = parse_i32(&attr),
            b"labelcols" => label.label_cols = parse_u16(&attr),
            b"labelrows" => label.label_rows = parse_u16(&attr),
            b"landscape" => label.landscape = attr_str(&attr),
            b"pagewidth" => label.page_width = parse_i32(&attr),
            b"pageheight" => label.page_height = parse_i32(&attr),
            _ => {}
        }
    }
    label
...
fn synthesize_common_obj_attr(common: &CommonObjAttr) -> u32 {
    ...
    attr |= match common.text_wrap {
        ...
        TextWrap::Tight | TextWrap::Through => 0,
    } << 21;
    attr
}
...
fn parse_table_cell(
    ...
    for attr in e.attributes().flatten() {
        match attr.key.as_ref() {
            b"borderFillIDRef" => cell.border_fill_id = parse_border_fill_ref(&attr),
            b"header" => cell.is_header = attr_str(&attr) == "1",
            b"hasMargin" => cell.apply_inner_margin = parse_bool(&attr),
            b"textDirection" => {
                let val = attr_str(&attr);
                ...
                match local {
                    ...
                    b"tcPr" => {
                        ...
                        for attr in ce.attributes().flatten() {
                            match attr.key.as_ref() {
                                b"borderFillIDRef" => {
                                    cell.border_fill_id = parse_border_fill_ref(&attr)
                                }
                                b"textDirection" => {
                                    let val = attr_str(&attr);
                                    cell.text_direction = if val == "VERTICAL" { 1 } else { 0 };
                        ...
                        for attr in ce.attributes().flatten() {
                            match attr.key.as_ref() {
                                b"x" => {
                                    ...
                                    if !has_pos {
                                        common.horizontal_offset = v.max(0) as u32;
                                    }
                                }
                                b"y" => {
                                    let v = parse_i32(&attr);
                                    ...
                                    if !has_pos {
                                        common.vertical_offset = v.max(0) as u32;
                                    }
                                }
                                _ => {}
                            }
            ...
            Ok(Event::Empty(ref ce)) | Ok(Event::Start(ref ce)) => {
                ...
                match local {
                    b"winBrush" => {
                        ...
                        let mut hatch_is_none = false;
                        for attr in ce.attributes().flatten() {
                            match attr.key.as_ref() {
                                b"faceColor" => {
                                    let value = attr_str(&attr);
                                    face_is_none = value == "none" || value.is_empty();
                                    solid.background_color = parse_color(&attr);
                                }
                                b"hatchColor" => {
                                    let value = attr_str(&attr);
                                    hatch_is_none = value == "none" || value.is_empty();
                                    solid.pattern_color = parse_color(&attr);
                                }
                                b"alpha" => {
                                    let val = attr_str(&attr);
...
fn parse_shape_object(
    ...
    let mut round_rate: u8 = 0;
    // hc:startPt / hc:endPt — LineShape/ArcShape 의 양 끝점. (HWPX 정규 좌표,
    // shape_attr.original_width/height 기준 정규화).
    let mut start_pt = Point::default();
    let mut end_pt = Point::default();
    let mut line_reverse_hv = false;

    parse_object_element_attrs(e, &mut common, &mut shape_attr);
    for attr in e.attributes().flatten() {
        match attr.key.as_ref() {
            b"ratio" => round_rate = parse_u8(&attr),
            b"startX" => start_pt.x = parse_i32(&attr),
            b"startY" => start_pt.y = parse_i32(&attr),
            b"endX" => end_pt.x = parse_i32(&attr),
            b"endY" => end_pt.y = parse_i32(&attr),
            b"isReverseHV" => line_reverse_hv = parse_u8(&attr) != 0,
            _ => {}
        }
    }
            ...
            Ok(Event::Start(ref ce)) | Ok(Event::Empty(ref ce)) => {
                ...
                match local {
                    ...
                    b"pt" => {
                        ...
                        for attr in ce.attributes().flatten() {
                            match attr.key.as_ref() {
                                b"x" => pt.x = parse_i32(&attr),
                                b"y" => pt.y = parse_i32(&attr),
                                _ => {}
                            }
                        }
                    ...
                    b"startPt" => {
                        // <hc:startPt x="..." y="..."/>  — LineShape/ArcShape 의 시작점.
                        // shape size (curSz) 와 곱해서 실제 좌표 derive. 위치 자체가
                        // HWPX 정규 좌표 (보통 0~original_width/height).
                        for attr in ce.attributes().flatten() {
                            match attr.key.as_ref() {
                                b"x" => start_pt.x = parse_i32(&attr),
                                b"y" => start_pt.y = parse_i32(&attr),
                                _ => {}
                            }
                        }
                    }
                    b"endPt" => {
                        ...
                        for attr in ce.attributes().flatten() {
                            match attr.key.as_ref() {
                                b"x" => end_pt.x = parse_i32(&attr),
                                b"y" => end_pt.y = parse_i32(&attr),
                                _ => {}
                            }
                        }
                    }
    ...
    // orgSz (design box, e.g. 100×100) — the space the line endpoints are
    // normalized to; captured before the fallback below mutates current_*.
    let org_w = shape_attr.original_width.max(1) as i64;
    let org_h = shape_attr.original_height.max(1) as i64;
    ...
    let shape = match shape_type {
        ...
        b"line" => {
            // HWPX normalizes line endpoints to orgSz (the design box, e.g.
            // 100×100); the rendered geometry scales them to the actual size
            // (`sz`/`curSz`, which is `common.width/height`). A vertical exam rule
            // stores endPt (100,100) with orgSz 100×100 and sz 1×81092, so the line
            // runs (0,0)→(1,81092). Without this scaling the endpoints stay at the
            // 100-unit design box and the rule collapses to an invisible nub.
            let bw = common.width as i64;
            let bh = common.height as i64;
            let scale = |p: Point| Point {
                x: (p.x as i64 * bw / org_w) as i32,
                y: (p.y as i64 * bh / org_h) as i32,
            };
            ShapeObject::Line(LineShape {
                common,
                drawing,
                start: scale(start_pt),
                end: scale(end_pt),
                started_right_or_bottom: line_reverse_hv,
                ..Default::default()
            })
        }
                ...
                match local {
                    ...
                    b"pic" => {
                        ...
                        if let Control::Picture(pic) = child {
                            let mut pic = pic;
                            normalize_group_child_common(&mut pic.common);
                    ...
                    b"rect" | b"ellipse" | b"line" | b"arc" | b"polygon" | b"curve" => {
                        ...
                        if let Control::Shape(shape) = child {
                            let mut shape = *shape;
                            normalize_group_child_local(&mut shape);
                            children.push(shape);
                        }
                    }
                    b"container" => {
...
fn normalize_group_child_local(shape: &mut ShapeObject) {
    normalize_group_child_common(shape.common_mut());
    if let ShapeObject::Group(group) = shape {
        for child in &mut group.children {
            normalize_group_child_local(child);
        }
    }
}

fn normalize_group_child_common(common: &mut CommonObjAttr) {
...
fn parse_ctrl(
    ...
    item_parts: &mut Vec<ParagraphItem>,
) -> Result<(), HwpxError> {
    let mut buf = Vec::new();
    loop {
        match reader.read_event_into(&mut buf) {
            Ok(Event::Start(ref ce)) => {
                ...
                match local {
                    b"colPr" => {
                        ...
                        item_parts.push(ParagraphItem::Control(controls.len()));
                    ...
                    b"autoNum" => {
                        ...
                        push_text_part(text_parts, item_parts, " ".to_string());
                    }
                    b"hiddenComment" => {
...
fn parse_page_num_attrs(e: &quick_xml::events::BytesStart) -> PageNumberPos {
    ...
    for attr in e.attributes().flatten() {
        match attr.key.as_ref() {
            ...
            b"formatType" => {
                pn.format = parse_number_format(&attr_str(&attr));
            }
            b"sideChar" => {
                let s = attr_str(&attr);
...
fn parse_number_format(s: &str) -> u8 {
    match s {
        "DIGIT" => 0,
        "CIRCLE_DIGIT" => 1,
        "ROMAN_CAPITAL" => 2,
        ...
        match reader.read_event_into(&mut buf) {
            Ok(Event::Start(ref ce)) | Ok(Event::Empty(ref ce)) => {
                ...
                if local == b"autoNumFormat" {
                    for attr in ce.attributes().flatten() {
                        match attr.key.as_ref() {
                            b"type" => an.format = parse_number_format(&attr_str(&attr)),
                            b"userChar" => {
                                let s = attr_str(&attr);
    ...
    loop {
        match reader.read_event_into(&mut buf) {
            ...
            Ok(Event::Text(ref t)) => {
                ...
                if matches!(current_string_param.as_deref(), Some("Command")) {
                    field.command = value;
                } else if matches!(current_integer_param.as_deref(), Some("Prop")) {
                    field.properties = value.parse().unwrap_or(field.properties);
                } else if matches!(current_integer_param.as_deref(), Some("Number")) {
                    field.memo_index = value.parse().unwrap_or(field.memo_index);
                }
            }
            Ok(Event::End(ref ee)) => {
                let eename = ee.name();
                let local = local_name(eename.as_ref());
                if local == b"stringParam" {
                    current_string_param = None;
                } else if local == b"integerParam" {
                    current_integer_param = None;
...
fn parse_equation(
    ...
    let equation = Equation {
        ...
        shape_original_width: 0,
        shape_original_height: 0,
        shape_current_width: 0,
        shape_current_height: 0,
    };
...
// ─── master page (Contents/masterpageN.xml) parser ──────────────────

/// `Contents/masterpageN.xml` 파일 1개를 파싱하여 `MasterPage` 모델로 변환한다.
///
/// 한컴 시험지 (수능/모의고사 양식) 의 헤더 ("수학 영역" 등), 페이지 우측
/// divider 선, 페이지번호 박스 (autoNum) 가 모두 master page 안에 있다.
/// rhwp 의 원래 hwpx loader 는 `Contents/masterpageN.xml` 자체를 읽지 않아
/// 시험지 페이지 헤더 영역이 통째로 비어있었다. 이 함수는 한컴 시험지
/// rendering 의 핵심 결손 해결을 위한 1차 진입점.
///
/// ## XML 구조
/// ```xml
/// <masterPage id="masterpage0" type="EVEN|ODD|BOTH"
    ...
    loop {
        match reader.read_event_into(&mut buf) {
            Ok(Event::Start(ref e)) => {
                let ename = e.name();
                let local = local_name(ename.as_ref());
                if local == b"masterPage" {
                    // root element attribute (type/pageNumber/...)
                    for attr in e.attributes().flatten() {
                        match attr.key.as_ref() {
                            b"type" => {
                                match attr_str(&attr).as_str() {
                                    ...
                                    _ => mp.apply_to = HeaderFooterApply::Both,
                                };
                            }
                            b"pageNumber" if mp.is_extension && mp.ext_flags == 0 => {
                                let page_number = parse_u32(&attr);
                                if page_number > 0 {
                                    mp.ext_flags = page_number.saturating_add(3) as u16;
                                }
                            }
                            _ => {}
                        }
                    ...
                    // subList attribute (textWidth/textHeight 보존)
                    for attr in e.attributes().flatten() {
                        match attr.key.as_ref() {
                    ...
                    // subList 안 paragraphs 파싱 (기존 함수 재사용)
                    let paras = parse_sublist_paragraphs(&mut reader, b"subList")?;
                    mp.paragraphs.extend(paras);
                }
            }
            Ok(Event::Eof) => break,
            Err(e) => return Err(HwpxError::XmlError(format!("master_page: {}", e))),
            _ => {}
        }
        buf.clear();
    }
...
<hs:sec xmlns:hp="http://www.hancom.co.kr/hwpml/2011/paragraph"
  ...
  <hp:p paraPrIDRef="0" styleIDRef="0">
    ...
    #[test]
    fn test_parse_table_cell_attrs_from_tc_element() {
        let xml = r#"<?xml version="1.0" encoding="UTF-8"?>
<hs:sec xmlns:hp="http://www.hancom.co.kr/hwpml/2011/paragraph"
        xmlns:hs="http://www.hancom.co.kr/hwpml/2011/section">
  <hp:p paraPrIDRef="0" styleIDRef="0">
    <hp:tbl rowCnt="1" colCnt="1" cellSpacing="0" borderFillIDRef="3">
      <hp:tr>
        <hp:tc name="0" header="1" hasMargin="1" editable="0" dirty="0"
               borderFillIDRef="5" textDirection="HORIZONTAL" vertAlign="CENTER"
               colAddr="2" rowAddr="3" colSpan="4" rowSpan="5" width="6000" height="7000">
          <hp:subList><hp:p paraPrIDRef="0" styleIDRef="0"><hp:run charPrIDRef="0"><hp:t>T</hp:t></hp:run></hp:p></hp:subList>
        </hp:tc>
      </hp:tr>
    </hp:tbl>
        ...
        assert_eq!(cell.row_span, 5);
        assert_eq!(cell.width, 6000);
        assert_eq!(cell.height, 7000);
