...
fn test_parse_page_def() {
    ...
    data.extend_from_slice(&0u32.to_le_bytes()); // attr (WIDELY, 한쪽)
...
fn test_parse_page_def_landscape() {
    ...
    data.extend_from_slice(&0x01u32.to_le_bytes()); // attr: NARROWLY
...
#[test]
fn test_trailing_master_page_list_header_does_not_leak_into_last_paragraph() {
    let mut section_bytes = Vec::new();

    let ph = make_para_header_data(5, 0, 0);
    let pt = make_para_text_data("BODY");
    ...
    list_header.extend_from_slice(&1u16.to_le_bytes()); // para_count
    list_header.extend_from_slice(&0u32.to_le_bytes()); // list_attr
    list_header.extend_from_slice(&0u16.to_le_bytes()); // width_ref
    list_header.extend_from_slice(&1000u32.to_le_bytes()); // text_width
    list_header.extend_from_slice(&1000u32.to_le_bytes()); // text_height
    list_header.push(0); // text_ref
    list_header.push(0); // num_ref
    list_header.extend_from_slice(&0u16.to_le_bytes()); // ext_flags
    section_bytes.extend(make_record_bytes(tags::HWPTAG_LIST_HEADER, 1, &list_header));

    let mph = make_para_header_data(7, 0, 0);
    let mpt = make_para_text_data("MASTER");
    section_bytes.extend(make_record_bytes(tags::HWPTAG_PARA_HEADER, 2, &mph));
    section_bytes.extend(make_record_bytes(tags::HWPTAG_PARA_TEXT, 3, &mpt));

    let mut auto_num = Vec::new();
    auto_num.extend_from_slice(&tags::CTRL_AUTO_NUMBER.to_le_bytes());
    auto_num.extend_from_slice(&0u32.to_le_bytes());
    auto_num.extend_from_slice(&1u16.to_le_bytes());
    auto_num.extend_from_slice(&0u16.to_le_bytes());
    auto_num.extend_from_slice(&0u16.to_le_bytes());
    auto_num.extend_from_slice(&0u16.to_le_bytes());
    section_bytes.extend(make_record_bytes(tags::HWPTAG_CTRL_HEADER, 1, &auto_num));
    ...
    assert_eq!(section.section_def.master_pages[0].paragraphs.len(), 1);
    assert_eq!(
        section.section_def.master_pages[0].paragraphs[0].text,
        "MASTER"
    );
}
