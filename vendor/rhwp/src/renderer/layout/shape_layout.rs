...
impl LayoutEngine {
    ...
    pub(crate) fn layout_shape(
        ...
        if let Control::Equation(eq) = ctrl {
            ...
            let (font_size_px, layout_box) =
                super::super::equation::build_equation_layout(eq, self.dpi);
    ...
    pub(crate) fn layout_textbox_content(
        ...
        if let Some(overflow_paras) = overflow_map.get(&key) {
            if text_direction != 0 {
                ...
                let composed_paras: Vec<_> = overflow_paras
                    .iter()
                    .map(|p| {
                        crate::renderer::composer::compose_paragraph_with_reflow(
                            p, styles, self.dpi,
                        )
                    })
                    .collect();
        ...
        let mut composed_paras: Vec<_> = text_box.paragraphs[..para_count]
            .iter()
            .map(|p| crate::renderer::composer::compose_paragraph_with_reflow(p, styles, self.dpi))
            .collect();
