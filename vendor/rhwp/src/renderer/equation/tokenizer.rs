...
impl Tokenizer {
    ...
    fn skip_spaces(&mut self) {
        // HWP 수식 스크립트는 HWPX 안에 XML 가독성용 `\r\n` 줄바꿈이 끼어 있는 경우가
        // 있다 (예: cases / eqalign 의 row 사이). 토큰 단위 처리에서는 줄바꿈을 일반
        // 공백처럼 흡수해야 — 그렇지 않으면 row 안에 보이지 않는 \r\n 글리프가 남아
        // 정렬·폭 계산이 흐트러진다 (Q9 piecewise 구간 정렬 어긋남 사례).
        while matches!(
            self.current(),
    ...
    /// 위치 `self.pos`부터 키워드가 prefix로 매치되는지 확인 (대소문자 무관)
    ...
    /// 경우에는 분리하지 않는다.
    /// 한컴 수식은 대소문자 구분 없음 — `RM`, `Rm`, `rm` 모두 동일하게 처리.
    fn read_command(&mut self) -> Token {
        let start = self.pos;

        // 폰트 스타일(`rm`/`it`/`bold`) + 데코레이션(`bar`/`vec`/`hat`/`tilde`/...)
        // prefix 분리. hwpeq 문법: `rmbarAB` 같이 명령어가 식별자에 공백 없이
        // 붙은 패턴을 명령어 단위로 분리한다. 긴 키워드 우선 매칭 (`bold` ↔ `bo`
        // 등 prefix 충돌 방지).
        for kw in [
            "bold", "tilde", "ddot", "check", "acute", "grave", "bar", "vec", "hat", "dot", "rm",
            "it",
        ] {
