...
fn parse_hwp_with_cfb(
    ...
    doc.legacy_equation_width_unit = normalize_hwp_equation_width_units(&mut doc);
...
fn normalize_hwp_equation_width_units(doc: &mut Document) -> bool {
    let mut stats = EquationWidthUnitStats::default();
    for section in &doc.sections {
        for para in &section.paragraphs {
            collect_equation_width_unit_stats(para, &mut stats);
        }
    }
    if !stats.uses_old_eq_width_unit() {
        return false;
    }
    for section in &mut doc.sections {
        for para in &mut section.paragraphs {
            normalize_equation_widths_in_para(para);
        }
    }
    true
}
...
#[derive(Default)]
struct EquationWidthUnitStats {
    total: usize,
    grid75: usize,
}

impl EquationWidthUnitStats {
    ...
    fn uses_old_eq_width_unit(&self) -> bool {
        self.total >= 10 && self.grid75 * 2 >= self.total
    }
}

fn collect_equation_width_unit_stats(para: &Paragraph, stats: &mut EquationWidthUnitStats) {
    for control in &para.controls {
        match control {
            Control::Equation(eq) => stats.push(eq.common.width),
            Control::Table(table) => {
                for cell in &table.cells {
                    for child in &cell.paragraphs {
                        collect_equation_width_unit_stats(child, stats);
                    }
                }
            }
...
fn normalize_equation_widths_in_para(para: &mut Paragraph) {
    for control in &mut para.controls {
        match control {
            Control::Equation(eq) => {
                eq.common.width = normalize_old_eq_width_unit(eq.common.width);
            }
            Control::Table(table) => {
                for cell in &mut table.cells {
                    for child in &mut cell.paragraphs {
                        normalize_equation_widths_in_para(child);
                    }
                }
            }
            Control::Header(header) => {
                for child in &mut header.paragraphs {
                    normalize_equation_widths_in_para(child);
                }
            }
            Control::Footer(footer) => {
                for child in &mut footer.paragraphs {
                    normalize_equation_widths_in_para(child);
                }
            }
            _ => {}
...
fn normalize_old_eq_width_unit(width: u32) -> u32 {
    ((width as u64 * 89_932 + 50_000) / 100_000) as u32
}
...
mod tests {
    ...
    fn test_parse_document_issue_265_sample() {
        ...
        assert!(
            ...
            "message should mention HWP 3.0: {msg}"
        );
        assert!(
            msg.contains("HWP 5.0"),
            "message should mention HWP 5.0: {msg}"
        );
    }
