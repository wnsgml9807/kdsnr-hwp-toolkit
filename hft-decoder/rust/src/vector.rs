...
pub fn walk_path(raw: &[u8]) -> Vec<PathCommand> {
    ...
    while pos < raw.len() {
        ...
        match op {
            0x00 | 0x04 => {
                if needs_close {
                    cmds.push(PathCommand { kind: CommandKind::Close, points: vec![] });
                    needs_close = false;
                    x = subpath_start_x;
                    y = subpath_start_y;
                }
            ...
            0x01 | 0x02 | 0x03 => {
                ...
                let mut dy = 0i32;
                if op & 1 != 0 {
                    match try_varint(raw, pos) { Some((v, p)) => { dx = v; pos = p; } None => break }
                }
                if op & 2 != 0 {
                    match try_varint(raw, pos) { Some((v, p)) => { dy = v; pos = p; } None => break }
                }
                x += dx;
                y += dy;
                subpath_start_x = x;
                subpath_start_y = y;
                cmds.push(PathCommand { kind: CommandKind::Move, points: vec![x, y] });
            }
            0x05 | 0x06 | 0x07 => {
                let mut dx = 0i32;
                let mut dy = 0i32;
                ...
                x += dx;
                y += dy;
                cmds.push(PathCommand { kind: CommandKind::Line, points: vec![x, y] });
                needs_close = true;
            }
            0x09 | 0x0A | 0x0B => {
                let mut dx0 = 0i32;
                let mut dy0 = 0i32;
                if op & 1 != 0 {
                    match try_varint(raw, pos) { Some((v, p)) => { dx0 = v; pos = p; } None => break }
                }
                if op & 2 != 0 {
                    match try_varint(raw, pos) { Some((v, p)) => { dy0 = v; pos = p; } None => break }
                }
                let d24;
                let d28;
                match try_varint(raw, pos) { Some((v, p)) => { d24 = v; pos = p; } None => break }
                match try_varint(raw, pos) { Some((v, p)) => { d28 = v; pos = p; } None => break }
                let mut d1c = 0i32;
                let mut d18 = 0i32;
                if op & 2 != 0 {
                    match try_varint(raw, pos) { Some((v, p)) => { d1c = v; pos = p; } None => break }
                }
                if op & 1 != 0 {
                    match try_varint(raw, pos) { Some((v, p)) => { d18 = v; pos = p; } None => break }
                }
                let cx1 = x + dx0;
                let cy1 = y + dy0;
                let cx2 = cx1 + d24;
            ...
            0x40 | 0x42 => {
                for _ in 0..2 {
                    match try_varint(raw, pos) { Some((_, p)) => { pos = p; } None => break }
                }
            }
            0x41 | 0x43 => {
                for _ in 0..5 {
