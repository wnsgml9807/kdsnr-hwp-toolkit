...
impl EqParser {
    ...
    /// 현재 토큰이 OVER/ATOP 이면 직전 element 와 다음 element 로 분수/atop 을
    /// 만들어 `current` 에 푸시. cases/matrix/eqalign/pile 의 셀 내부에서도
    /// 분수가 정상 동작하도록 공통 helper.
    fn try_consume_over_atop(&mut self, current: &mut Vec<EqNode>) -> bool {
        if self.current_type() != TokenType::Command {
            return false;
        }
        let is_over = Self::cmd_eq(self.current_value(), "OVER");
        let is_atop = Self::cmd_eq(self.current_value(), "ATOP");
        if !is_over && !is_atop {
            return false;
        }
        self.pos += 1;
        let top = current.pop().unwrap_or(EqNode::Empty);
        let bottom = self.parse_element();
        current.push(if is_atop {
            EqNode::Atop {
                top: Box::new(top),
        ...
        match ty {
            ...
            TokenType::Whitespace => {
                ...
                match val.as_str() {
                    ...
                    "#" => EqNode::Newline,
                    // EqAlign / Cases 환경 밖에서 `&` 가 단독으로 등장하면 한컴은
                    // 사실상 시각적 공백 없이(혹은 매우 좁게) 처리한다. 인라인 식에서
                    // Tab(=fs*1.0) 만큼 벌어지면 "AB    =1", "≤    -4" 처럼 비정상적인
                    // 큰 공백이 발생하므로 Empty 로 둔다 (EqAlign 본체는 별도 경로에서
                    // Tab 을 직접 push 하므로 영향 없음).
                    "&" => EqNode::Empty,
                    _ => EqNode::Empty,
                }
            }
    ...
    fn parse_command(&mut self, cmd: &str) -> EqNode {
        ...
        // LEFT-RIGHT 괄호
        if cu == "LEFT" {
            let paren = self.parse_left_right();
            // `LEFT(...)RIGHT}^{2}` 등 LEFT-RIGHT 직후의 첨자는 괄호에 attach 한다.
            return self.try_parse_scripts(paren);
        }
        ...
        // BOX: 시험지 빈칸 박스. body 를 사각형 frame 으로 둘러싼다.
        if cu == "BOX" {
            let body = self.parse_single_or_group();
            return EqNode::BoxFrame {
                body: Box::new(body),
            };
        }
        ...
        // 글꼴 스타일 — 한컴 수식은 대소문자 무관 (RM/Rm/rm 동일)
        let cmd_lower = cmd.to_ascii_lowercase();
        if let Some(&style) = FONT_STYLES.get(cmd_lower.as_str()) {
    ...
    fn parse_single_or_group(&mut self) -> EqNode {
        ...
        // 행/열 분리자 (`#`, `&`) 와 RIGHT 명령은 부모 구조 (cases/eqalign/pile/matrix
        // /LEFT-RIGHT) 의 토큰이므로 단일 body 로 흡수하면 안 된다. 흡수하면 부모의
        // row/col 분리가 깨지고 해당 분리자가 글리프로 떨어진다 (Q9 `pile{it#it}` 의
        // `it` font style 이 `#` 을 body 로 먹어 화면에 # 표시되던 사례).
        ...
        match ty {
            TokenType::Command => {
                ...
                } else if let Some(&deco) = DECORATIONS.get(val.as_str()) {
                    // `rm vec{AQ}` 처럼 FontStyle/구조 명령의 단일 body 가
                    // 또 다른 데코레이션 명령일 때 — 텍스트로 떨어뜨리면 안 되고
                    // 데코레이션 본체까지 같이 흡수해야 한다.
                    let body = self.parse_single_or_group();
                    EqNode::Decoration {
                        kind: deco,
                        body: Box::new(body),
    ...
    fn parse_matrix(&mut self, style: MatrixStyle) -> EqNode {
        ...
        while self.pos < end && !self.at_end() {
            ...
            if self.current_type() == TokenType::Whitespace && self.current_value() == "#" {
                ...
                current_cell = Vec::new();
                self.pos += 1;
            } else if self.try_consume_over_atop(&mut current_cell) {
                // handled
            } else {
    ...
    fn parse_cases(&mut self) -> EqNode {
        ...
        let end = self.find_matching_brace(self.pos);
        // rows: Vec<row>, row: Vec<cell> (셀 = `&` 로 구분되는 열)
        let mut rows: Vec<Vec<EqNode>> = Vec::new();
        let mut current_row: Vec<Vec<EqNode>> = vec![Vec::new()];
        // pending_extra_amp: 셀 사이에 `&&` 추가분이 있을 때 다음 셀 앞에 Tab 삽입
        let mut pending_extra_amp: usize = 0;
        ...
        while self.pos < end && !self.at_end() {
            ...
            if self.current_type() == TokenType::Whitespace && self.current_value() == "#" {
                let row: Vec<EqNode> = current_row
                    .into_iter()
                    .map(|cell| EqNode::Row(cell).simplify())
                    .collect();
                rows.push(row);
                current_row = vec![Vec::new()];
                pending_extra_amp = 0;
                self.pos += 1;
            } else if self.current_type() == TokenType::Whitespace && self.current_value() == "&" {
                // `&` 갯수 카운트: 1개 = 새 셀 시작; 2개 이상 = 새 셀 시작 + 추가 Tab 공간
                ...
                // 두 번째 이후의 `&` 는 다음 셀 앞에 Tab 으로 추가
                pending_extra_amp = amp_count.saturating_sub(1);
                for _ in 0..pending_extra_amp {
                    if let Some(cell) = current_row.last_mut() {
                        cell.push(EqNode::Space(super::ast::SpaceKind::Tab));
                    }
                }
                pending_extra_amp = 0;
            } else if self.try_consume_over_atop(current_row.last_mut().unwrap()) {
                // handled
            } else {
                let elem = self.parse_element();
                current_row.last_mut().unwrap().push(elem);
            }
        }

        // 마지막 행
        let last_row_nonempty = current_row.iter().any(|c| !c.is_empty());
        if last_row_nonempty {
            let row: Vec<EqNode> = current_row
                .into_iter()
                .map(|cell| EqNode::Row(cell).simplify())
                .collect();
    ...
    fn parse_pile(&mut self, align: PileAlign) -> EqNode {
        ...
        while self.pos < end && !self.at_end() {
            ...
            if self.current_type() == TokenType::Whitespace && self.current_value() == "#" {
                ...
                current_row = Vec::new();
                self.pos += 1;
            } else if self.try_consume_over_atop(&mut current_row) {
                // handled
            } else {
    ...
    fn parse_eqalign(&mut self) -> EqNode {
        ...
        while self.pos < end && !self.at_end() {
            ...
            if self.current_type() == TokenType::Whitespace && self.current_value() == "#" {
                ...
                let target = if let Some(ref mut right) = current_right {
                    right
                } else {
                    &mut current_left
                };
                if !self.try_consume_over_atop(target) {
                    target.push(self.parse_element());
                }
            }
        }
