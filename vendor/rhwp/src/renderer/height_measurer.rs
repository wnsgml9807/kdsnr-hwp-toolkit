...
impl HeightMeasurer {
    ...
    fn measure_paragraph(
        ...
        // 2026-05-19: paragraph_layout 와 동일하게 inline equation/shape 처리.
        // affectLSpacing=0 분수/도형이 paragraph 안에 있으면 한컴은 모든 lineseg.vs 를
        // 분수/도형 height 로 저장한다. raw_lh 를 그대로 사용하면 paragraph height 가
        // 부풀려져 pagination 단에서 페이지 사용량이 한컴 GT 와 어긋난다 (dump-pages
        // diff=+291.5px Q03 케이스).
        let has_tac_eq_or_shape = para.controls.iter().any(|c| match c {
            crate::model::control::Control::Equation(eq) => eq.common.treat_as_char,
            ...
            comp.lines
                ...
                .map(|line| {
                    ...
                    let max_fs = line
                        ...
                        .fold(0.0f64, f64::max);
                    let lh = if has_tac_eq_or_shape && raw_lh > max_fs * 1.5 {
                        max_fs * 1.2 // paragraph_layout 의 has_tac_shape/eq 와 동일 보정
                    } else {
        ...
        let lines_total: f64 = {
            let sum: f64 = line_heights
                ...
                .map(|(h, s)| h + s)
                .sum();
            // 2026-05-19: 한컴 stored LineSeg.vertical_pos 가 paragraph height 의 진실값.
            // paragraph_layout 도 stored vp 기반으로 누적하므로 (use_stored_vp),
            // height_measurer 도 동일 식으로 일치시켜야 pagination current_height 와
            // paragraph_layout return y 가 정합한다.
            // paragraph_h = last.vp + last.lh + last.ls - first.vp
            // stored vp 가 모두 0 인 빈 paragraph 만 일반 누적 fallback.
