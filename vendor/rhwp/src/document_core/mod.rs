...
pub struct DocumentCore {
    ...
    /// HFT 글리프 캐시 (`--hft-path` 활성 시 채워짐). SvgRenderer 가 cache
    /// hit 시 `<text>` 대신 `<path>` emit. 상세는
    /// kdsnr-hwp-toolkit/work/hft_re/INTEGRATION_CONTEXT.md 섹션 18.
    #[cfg(not(target_arch = "wasm32"))]
    pub(crate) hft_cache: Option<std::sync::Arc<kdsnr_hft::HftCache>>,
}
...
impl DocumentCore {
    ...
    /// HFT 글리프 캐시 설정. native (non-WASM) 빌드 전용.
    #[cfg(not(target_arch = "wasm32"))]
