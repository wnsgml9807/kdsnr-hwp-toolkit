...
impl AliasMap {
    ...
    fn insert_alias(&mut self, name: &str, entry: FaceEntry) {
        ...
        if !v
            .iter()
            .any(|e| e.hft == entry.hft && e.category == entry.category)
        {
            v.push(entry);
        }
    }
    ...
    ///
    /// Hancom PDF export RE policy:
    /// - Pick the HFT sibling by Unicode category from `hftinfo.dat`.
    /// - This includes Latin.  Windows Frida full capture during COM
    ///   `SaveAs(PDF)` showed ASCII/punctuation taking the same
    ///   `AC080 -> AD2C0` HFT path as Hangul, e.g. `신명 중명조` digits use
    ///   `TEJMJEN.HFT` and `한양신명조` punctuation uses `ENSMJ.HFT`.
    /// - Direct canonical HFT names (e.g. `cache.get("TEJMJHG", ch)`) are still
    ///   handled before alias lookup in `HftCache::get`.
...
pub fn category_for_code(code: u32) -> FaceCategory {
    match code {
        // ASCII + Latin-1 Supplement + Latin Extended
