...
fn parse_para_shape(data: &[u8]) -> Result<ParaShape, DocInfoError> {
    ...
    // condense 는 HWP 5.0 binary spec 에 명시적 필드가 없는 hwpx-전용 속성.
    // 한컴은 hwpx export 시 본문 paragraph 에 50 을 박는다 (실측 — 71개 중
    // 62개 = 50, 9개 = 0 [빈 paragraph / `<보기>` 헤더]). binary 출처에는
    // 정보가 없으므로 default 50 으로 채워 hwpx 출력 시 한컴 기본값과 일치
    // 시킨다.
    let condense = 50u8;
