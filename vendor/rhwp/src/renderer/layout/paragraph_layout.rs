...
/// HWPX 텍스트 내 문자가 차지하는 UTF-16 code unit 크기.
///
/// 일반 BMP 문자는 1, 서로게이트(supplementary plane)는 2, 그리고
/// HWPX 가 텍스트에 그대로 박아넣지만 16바이트(8 코드유닛) 자리를
/// 점유하는 확장 컨트롤 문자(`\t` 등)는 8 을 돌려준다.
///
/// 이 값은 `char_offsets` 의 연속 차분과 직접 비교되며, segment splitter 와
/// line-break char index 변환이 동일한 규칙으로 동작하게 한다.
pub(crate) fn hwpx_char_unit_size(c: char) -> u32 {
    match c {
        // HWP 0x0009 TAB: parser 가 16바이트 = 8 코드유닛으로 advance
        '\t' => 8,
        c if c >= '\u{10000}' => 2,
        _ => 1,
    }
...
impl LayoutEngine {
    pub(crate) fn layout_inline_table_paragraph(
        ...
        // ParaShape 의 tab 관련 필드 — page-level 경로와 동일하게 inline-table 경로도 소비.
        let para_tab_width = para_style.map(|s| s.default_tab_width).unwrap_or(0.0);
        let para_tab_stops = para_style.map(|s| s.tab_stops.clone()).unwrap_or_default();
        let para_auto_tab_right = para_style.map(|s| s.auto_tab_right).unwrap_or(false);

        // 첫 줄 indent (HWP hanging/first-line indent 시멘틱):
        //  - indent > 0 (들여쓰기): 첫 줄에 + indent
        //  - indent < 0 (내어쓰기): 첫 줄에 0, 후속 줄에 + abs(indent) — 단 inline-table 단락은
        //    대부분 single-line 이라 wrapping 의 영향이 적음. 줄바꿈 case 는 wrapped_below_table
        //    분기에서 별도 처리.
        let first_line_indent = if indent > 0.0 { indent } else { 0.0 };
        let wrap_line_indent = if indent < 0.0 { indent.abs() } else { 0.0 };

        let y = y_start + spacing_before;
        if std::env::var("RHWP_LAYOUT_DEBUG").is_ok() {
            eprintln!(
                "LAYOUT_INLINE_TAC pi={} y_start={:.2} spacing_before={:.2} y={:.2}",
                para_index, y_start, spacing_before, y
            );
        }
        ...
        for i in 1..offsets.len() {
            let prev_char_utf16_len = hwpx_char_unit_size(text_chars[i - 1]);
            let gap = offsets[i] - offsets[i - 1];
            if gap > prev_char_utf16_len + 4 {
                // 갭에 컨트롤이 있음 (인라인 표 등 — \t 는 자체가 8 코드유닛이므로 갭으로 잡히지 않음)
        ...
        // composed.tab_extended 는 paragraph 내 `\t` 의 순서대로 정렬된 inline tab 정보.
        // estimate_text_width 는 호출당 tab_char_idx=0 부터 inline_tabs 를 읽으므로,
        // char-by-char 측정 시 매번 슬라이스 시작점을 옮겨줘야 올바른 너비가 적용된다.
        let tab_extended: &[[u16; 7]] = composed.map(|c| c.tab_extended.as_slice()).unwrap_or(&[]);

        // 세그먼트 폭 측정 — TabDef stop 매칭이 누적 위치에 의존하므로 stateful 루프.
        // cumulative_local_x 는 line-start 부터의 현재 cursor 위치 (margin_left + first_line_indent 기준).
        let available_width_est = col_area.width - margin_left - margin_right;
        let initial_local_x = margin_left + first_line_indent;
        let mut cumulative_local_x = initial_local_x;
        let mut seg_tab_idx = 0usize;
        let mut seg_widths: Vec<f64> = Vec::with_capacity(segments.len());
        for (seg_i, (s, e)) in segments.iter().enumerate() {
            ...
            for ch_idx in *s..*e {
                ...
                ts.default_tab_width = para_tab_width;
                ts.tab_stops = para_tab_stops.clone();
                ts.auto_tab_right = para_auto_tab_right;
                ts.available_width = available_width_est;
                ts.line_x_offset = cumulative_local_x + seg_total;
                if ch == '\t' {
                    ts.inline_tabs = tab_extended
                        .get(seg_tab_idx..)
                        .map(|s| s.to_vec())
                        .unwrap_or_default();
                    seg_tab_idx += 1;
                }
                seg_total += estimate_text_width(&ch.to_string(), &ts);
            }
            seg_widths.push(seg_total);
            cumulative_local_x += seg_total;
            // 이 세그먼트 뒤에 inline-table 이 오면 그 너비도 cursor 에 더한다.
            if seg_i < table_widths.len() {
                cumulative_local_x += table_widths[seg_i];
            }
        }
        ...
        // 첫 줄에 들여쓰기 (positive indent) 만큼 시작 x 를 우측으로 밀어준다.
        // hanging (negative indent) 는 후속 줄에 영향. inline-table 단락은 대부분 single-line.
        let start_x = match alignment {
            Alignment::Center | Alignment::Distribute => {
                col_area.x
                    + margin_left
                    + first_line_indent
                    + (available_width - total_width - first_line_indent).max(0.0) / 2.0
            }
            Alignment::Right => {
                col_area.x
                    + margin_left
                    + first_line_indent
                    + (available_width - total_width - first_line_indent).max(0.0)
            }
            _ => col_area.x + margin_left + first_line_indent,
        };
        ...
        let line_break_char_idx: Option<usize> = if para.line_segs.len() > 1 {
            ...
            if !para.char_offsets.is_empty() {
                ...
                for i in 1..para.char_offsets.len() {
                    let prev_len = hwpx_char_unit_size(text_chars[i - 1]);
        ...
        let mut wrapped_below_table = false; // 텍스트가 표 아래로 줄바꿈되었는지
                                             // 렌더링 패스용 별도 tab idx (seg_widths 와 독립적으로 진행)
        let mut render_tab_idx = 0usize;
            ...
            if *s < *e {
                ...
                if !seg_text.is_empty() {
                    ...
                    let mut line_run_x = inline_x; // 현재 줄 run의 x 시작
                                                   // 현재 줄 run 의 inline_tabs 시작 인덱스 (run 내 \t 들이 올바른 tab_extended 슬롯을 쓰도록)
                    let mut line_run_start_tab_idx = render_tab_idx;
                    ...
                    for ch_idx in *s..*e {
                        ...
                        let lang = super::super::style_resolver::detect_lang_category(ch);
                        let mut ts = resolved_to_text_style(styles, cs_id, lang);
                        // ParaShape 의 탭 관련 필드 주입 — TabDef stop 매칭 + RIGHT/CENTER 탭 시멘틱 활성화.
                        ts.default_tab_width = para_tab_width;
                        ts.tab_stops = para_tab_stops.clone();
                        ts.auto_tab_right = para_auto_tab_right;
                        ts.available_width = available_width;
                        ts.line_x_offset = inline_x - col_area.x;
                        // tab idx 는 측정용 ts 에만 슬라이스로 주입. 실제 카운터 증가는 run flush 후 (아래 참조).
                        if ch == '\t' {
                            ts.inline_tabs = tab_extended
                                .get(render_tab_idx..)
                                .map(|s| s.to_vec())
                        ...
                        if (cs_changed || need_wrap) && ch_idx > line_run_start {
                            ...
                            let mut run_ts =
                                resolved_to_text_style(styles, current_cs_id, first_lang);
                            run_ts.default_tab_width = para_tab_width;
                            run_ts.tab_stops = para_tab_stops.clone();
                            run_ts.auto_tab_right = para_auto_tab_right;
                            run_ts.available_width = available_width;
                            run_ts.line_x_offset = line_run_x - col_area.x;
                            run_ts.inline_tabs = tab_extended
                                .get(line_run_start_tab_idx..)
                                .map(|s| s.to_vec())
                                .unwrap_or_default();
                            let run_width = estimate_text_width(&run_text, &run_ts);
                            ...
                            col_node.children.push(run_node);
                            line_run_start = ch_idx;
                            line_run_start_tab_idx = render_tab_idx;
                            line_run_x = inline_x;
                        }
                    ...
                    if line_run_start < *e {
                        ...
                        let mut run_ts = resolved_to_text_style(styles, current_cs_id, first_lang);
                        run_ts.default_tab_width = para_tab_width;
                        run_ts.tab_stops = para_tab_stops.clone();
                        run_ts.auto_tab_right = para_auto_tab_right;
                        run_ts.available_width = available_width;
        ...
        let ret = effective_line_bottom + spacing_after;
        if std::env::var("RHWP_LAYOUT_DEBUG").is_ok() {
            eprintln!("LAYOUT_INLINE_TAC pi={} ret y={:.2} max_tbl_bot={:.2} text_bot={:.2} (y={:.2} lh={:.2} ls={:.2}) spacing_after={:.2}",
                para_index, ret, max_table_bottom, text_bottom, y, line_height, line_spacing, spacing_after);
        }
        ret
    }
    ...
    pub(crate) fn layout_paragraph(
        ...
        // body paragraph 의 빈 lineseg 정상화는 export 진입점의
        // `reflow_linesegs_on_demand` (rhwp `--reflow` flag) 에서 한 번에 처리한다.
        // 본 함수는 정상화된 lineseg 를 그대로 layout 한다.
    ...
    pub(crate) fn layout_composed_paragraph(
        ...
        if std::env::var("RHWP_LAYOUT_DEBUG").is_ok() {
            let has_tac = para
                ...
                .unwrap_or(false);
            eprintln!(
                "LAYOUT_PARA pi={} y_start={:.2} lines={} has_tac_table={}",
                para_index,
                y_start,
                composed.lines.len(),
                has_tac
            );
            for (i, l) in composed.lines.iter().enumerate() {
                eprintln!(
                    "  line[{}] vp={} lh={} bd={} sw={} runs={}",
                    i,
                    l.vertical_pos,
                    l.line_height,
                    l.baseline_distance,
        ...
        let (inner_pad_left, inner_pad_right) =
        ...
        // 2026-05-19: 한컴 stored LineSeg.vertical_pos 가 paragraph 안 line 위치의
        // 진실값. composer 가 lineseg.vp 를 그대로 복사 (composer.rs:363/384/419)
        // 하므로 stored vp 를 모든 paragraph 에 적용한다.
        //
        // 이전: has_inline_tac_table 인 경우만 stored vp 사용 (Q29 fix).
        //   문제: 일반 paragraph 는 line_height + line_spacing 누적이지만,
        //   corrected_line_height 가 raw_lh < max_fs 일 때 max_fs * ls_val/100 으로
        //   부풀려 stored vp gap 보다 크게 누적 → dump-pages 의 diff=+128.6px
        //   (한컴 GT 자체도) 발생.
        //
        // stored vp 가 모두 0 (lineseg 미정상화) 인 경우만 line_height 누적 fallback.
        // 첫 line vp 와 마지막 line vp 중 하나라도 > 0 이면 stored 사용.
        let stored_vp_base = composed
            .lines
            .get(start_line)
            .map(|l| l.vertical_pos)
            .unwrap_or(0);
        let last_line_idx_for_vp = end.saturating_sub(1).max(start_line);
        let stored_vp_last = composed
            .lines
            .get(last_line_idx_for_vp)
            .map(|l| l.vertical_pos)
            .unwrap_or(0);
        let use_stored_vp = stored_vp_base != 0 || stored_vp_last != 0;
        let y_para_origin = y; // spacing_before 적용 후 paragraph 시작 y
        ...
        for line_idx in start_line..end {
            ...
            let has_tac_shape = !tac_offsets_px.is_empty()
                && para
                    ...
                    .unwrap_or(false);
            // 2026-05-19: 인라인 Equation(수식) 도 동일 패턴.
            // paragraph 안에 affectLSpacing=0 분수가 있으면 한컴은 모든 line 의 lineseg.vs
            // 를 분수 height 로 저장한다. rhwp 가 vs 그대로 line_height 로 쓰면 (1) 분수
            // 없는 첫 줄도 line_height 가 분수만큼 커져 두 줄 간격이 크게 벌어지고,
            // (2) 셀 안 paragraph 면 셀 height 도 부풀려진다.
            // Shape 와 달리 text_y 이동은 안 함 (분수는 baseline 정렬이므로 text 는 일반 위치).
            let has_tac_eq = !tac_offsets_px.is_empty()
                && para
                    .map(|p| {
                        tac_offsets_px.iter().any(|(_, _, ci)| {
                            p.controls
                    ...
                    .unwrap_or(false);
            let (line_height, baseline) = if (has_tac_shape || has_tac_eq) && raw_lh > max_fs * 1.5
            {
                // Shape/Eq 높이가 line_height에 포함 → 폰트 기반 line_height 사용
            ...
            let mut effective_margin_left = margin_left + line_indent;
            let mut effective_margin_right = margin_right;

            // KDSNR_STYLE_PROBE — 첫 4개 paragraph 의 line 0 의 run 별 dispatch 검증
            #[cfg(not(target_arch = "wasm32"))]
            if std::env::var("RHWP_KDSNR_STYLE").is_ok() && line_idx == 0 && para_index <= 4 {
                for run in &comp_line.runs {
                    let rcs = styles.char_styles.get(run.char_style_id as usize);
                    let preview: String = run.text.chars().take(10).collect();
                    if let Some(c) = rcs {
                        eprintln!(
                            "KDSNR_STYLE pi={} cs_id={:>3} fs={:.2}px fam={:?} bold={} text={:?}",
                            para_index,
                            run.char_style_id,
                            c.font_size,
                            c.font_family,
                        ...
                        eprintln!(
                            "KDSNR_STYLE pi={} cs_id={} (no resolved)",
                            para_index, run.char_style_id
                        );
                    }
                }
            }

            // KDSNR_LAYOUT_PROBE — 모든 line 에서 kdsnr_bridge::compose_line 호출,
            // 결과 CharItemView width sum 과 rhwp segment_width 비교 dump.
            // G-W-1 wire PoC. 추후 결과로 TextRun bbox 갱신.
            #[cfg(not(target_arch = "wasm32"))]
            if std::env::var("RHWP_USE_KDSNR_LAYOUT").is_ok() {
                use crate::renderer::kdsnr_bridge::{
                    compose_line, default_horizontal_body_property,
                    resolved_char_style_to_property, resolved_para_style_to_property,
                    run_to_civs_with_raw_ratio,
                };

                if let Some(ps) = para_style {
                    let pp = resolved_para_style_to_property(ps);
                    let bp = default_horizontal_body_property();
                    let mut all_civs = Vec::new();
                    let mut first_diag_done = false;
                    for run in &comp_line.runs {
                        if let Some(rcs) = styles.char_styles.get(run.char_style_id as usize) {
                            let rp = resolved_char_style_to_property(rcs);
                            let font_family = if rcs.font_family.is_empty() {
                                "HBatang"
                            } else {
                                rcs.font_family.as_str()
                            };
                            let raw_family = rcs.raw_font_family.as_str();
                            // 1회만 — 첫 char 의 advance를 3 갈래 path 로 측정
                            if !first_diag_done && para_index <= 1 && line_idx <= 1 {
                                if let Some(first_c) = run.text.chars().next() {
                                    let hft_raw = crate::renderer::kdsnr_hft_global::advance_em(
                                        raw_family, first_c,
                                    );
                                    let hft_sub = crate::renderer::kdsnr_hft_global::advance_em(
                                        font_family,
                                        first_c,
                                    );
                                    let mca = crate::renderer::font_runtime_metrics::measure_char_advance_em(
                                        font_family, rcs.bold, rcs.italic, first_c);
                                    let text_style = resolved_to_text_style(
                                        styles,
                                        run.char_style_id,
                                        run.lang_index,
                                    );
                                    let etw =
                                        estimate_text_width(&first_c.to_string(), &text_style);
                                    eprintln!(
                                        "PATH_DIAG pi={} line={} c={:?} raw_fam={:?} sub_fam={:?} | HFT(raw)={:?} HFT(sub)={:?} measure_em={:?} etw1px={:.3} fs={:.2}",
                                        para_index, line_idx, first_c, raw_family, font_family,
                                        hft_raw, hft_sub, mca, etw, rcs.font_size,
                                    );
                                    first_diag_done = true;
                                }
                            }
                            let civs = run_to_civs_with_raw_ratio(
                                run,
                                &rp,
                                &pp,
                                &bp,
                                ...
                                rcs.bold,
                                rcs.italic,
                                rcs.font_size as f32,
                                rcs.ratio as f32,
                                rcs.letter_spacing as f32,
                            );
                            all_civs.extend(civs);
                        }
                    }
                    if !all_civs.is_empty() {
                        let kdsnr_w_sum: f32 = all_civs.iter().map(|c| c.width).sum();
                        let n_in = all_civs.len();
                        let result = compose_line(all_civs, 1);
                        let body_n = result.items.len().saturating_sub(4);
                        // rhwp 측 line width (각 run.estimate_text_width 합산, px 단위)
                        let mut rhwp_w_sum: f64 = 0.0;
                        for run in &comp_line.runs {
                            let ts =
                                resolved_to_text_style(styles, run.char_style_id, run.lang_index);
                            rhwp_w_sum += estimate_text_width(&run.text, &ts);
                        }
                        let rhwp_sw_px = hwpunit_to_px(comp_line.segment_width, self.dpi);
                        let preview: String = comp_line
                            .runs
                            .iter()
                            ...
                            .collect();
                        eprintln!(
                            "KDSNR_LAYOUT pi={} line={} chars={} kdsnr_w={:.2}px rhwp_text_w={:.2}px rhwp_sw_px={:.2} (sw_hu={}) text={:?}",
                            para_index, line_idx, n_in, kdsnr_w_sum, rhwp_w_sum,
                            rhwp_sw_px, comp_line.segment_width, preview,
                        );
                        let _ = body_n; // mute unused
                    }
                }
            }
            ...
            let (effective_col_x, effective_col_w) = if cell_ctx.is_some()
                && comp_line.segment_width > 0
                && comp_line.column_start > 0
            {
                // cs > 0: 한컴이 lineSeg.horzpos 에 indent 까지 미리 반영해 둔 경우.
                // 이 값을 신뢰하고 effective_margin_left 를 0 으로 리셋한다.
                // 일부 보기 박스 (cellMargin + paragraph indent 가 모두 적용된 케이스) 가 여기 해당.
                //
                // RIGHT-PADDING-CLAMP: caller (table layout) 가 col_area 를 cell inner_area 로
                // 좁혀서 전달한다는 contract 가 [paragraph_layout:1182 주석] 에 명시되어 있다.
                // cs/sw 가 한컴 native 이지만, sw 가 col_area.width 보다 크면 right padding 침범.
                // col_area inner 안에 clamp 하여 right padding 보장.
                let cs_px = hwpunit_to_px(comp_line.column_start, self.dpi);
                let sw_px = hwpunit_to_px(comp_line.segment_width, self.dpi);
                let inner_left = col_area.x;
                let inner_right = col_area.x + col_area.width;
                let ex = (col_area.x + cs_px).max(inner_left).min(inner_right);
                let ew = ((col_area.x + cs_px + sw_px).min(inner_right) - ex).max(0.0);
                effective_margin_left = 0.0;
                effective_margin_right = 0.0;
                (ex, ew)
            } else if cell_ctx.is_some() && comp_line.segment_width > 0 {
                // cs == 0: lineSeg 가 indent 를 인코딩하지 않은 raw 상태 (probe: Q20 cell.p[*]
                // 의 모든 lineSeg.cs=0 이지만 ParaShape.indent ≠ 0). 셀 inner 너비만
                // sw 에서 가져오고 margin_left + line_indent 는 그대로 적용한다.
                //
                // RIGHT-PADDING-CLAMP: 동일 이유로 sw 가 col_area.width 초과 시 clamp.
                let sw_px = hwpunit_to_px(comp_line.segment_width, self.dpi);
                let ew = sw_px.min(col_area.width).max(0.0);
                (col_area.x, ew)
            } else if has_picture_shape_square_wrap
                && comp_line.segment_width > 0
            ...
            let text_y = if use_stored_vp {
                y_para_origin + hwpunit_to_px(comp_line.vertical_pos - stored_vp_base, self.dpi)
            ...
            let line_fit_scale = if cell_ctx.is_some()
                && !has_tabs
                && effective_text_width > available_width + 0.5
                && effective_text_width > 0.0
            {
                (available_width / effective_text_width).clamp(0.85, 1.0)
            } else {
                1.0
            };
            ...
            for (run_idx, run) in comp_line.runs.iter().enumerate() {
                ...
                if line_fit_scale < 1.0 {
                    text_style.ratio *= line_fit_scale;
                }
                ...
                let full_width = if run.char_overlap.is_some() {
                    ...
                    // rhwp `estimate_text_width` 와 동일 path 지만 한컴 byte-eq metric 우선 (HFT
                    // raw face name + ratio + letter_spacing + HWPUNIT quantize).
                    // 일반 텍스트 line 에서 측정 차이 <1% (PoC 입증).
                    // RHWP_DISABLE_KDSNR_WIDTH=1 로 별도 비활성화 가능 (pixel diff A/B 용).
                    #[cfg(not(target_arch = "wasm32"))]
                    let kdsnr_w: Option<f64> = if std::env::var("RHWP_USE_KDSNR_LAYOUT").is_ok()
                        && std::env::var("RHWP_DISABLE_KDSNR_WIDTH").is_err()
                        && !run.text.contains('\t')
                    {
                        styles
                            .char_styles
                            .get(run.char_style_id as usize)
                            .and_then(|rcs| para_style.map(|ps| (rcs, ps)))
                            .map(|(rcs, ps)| {
                                use crate::renderer::kdsnr_bridge::{
                                    default_horizontal_body_property,
                                    resolved_char_style_to_property,
                                    resolved_para_style_to_property, run_to_civs_with_raw_ratio,
                                };
                                let rp = resolved_char_style_to_property(rcs);
                                let pp = resolved_para_style_to_property(ps);
                                let bp = default_horizontal_body_property();
                                ...
                                civs.iter().map(|c| c.width as f64).sum::<f64>()
                            })
                    } else {
                        None
                    };
                    #[cfg(target_arch = "wasm32")]
                    let kdsnr_w: Option<f64> = None;
                ...
                // 마지막 run 또는 line-break 로 끝나는 마지막 run 에서는
                // run_char_end 위치의 TAC 도 포함 (강제 줄넘김 직전 수식이 누락되는 버그 방지).
                let include_boundary = is_last_run || is_line_break;
                let run_tacs: Vec<(usize, f64, usize)> = tac_offsets_px
                    .iter()
                    .filter(|(pos, _, _)| {
                        *pos >= run_char_pos
                            && (*pos < run_char_end || (include_boundary && *pos == run_char_end))
                    })
                    ...
                    for &(tac_rel, tac_w, tac_ci) in &run_tacs {
                        ...
                        if let Some(p) = para {
                            if let Some(Control::Equation(eq)) = p.controls.get(tac_ci) {
                                ...
                                // (font size 는 sz height 에 맞춰 자동 스케일링됨)
                                let (font_size_px, layout_box) =
                                    crate::renderer::equation::build_equation_layout(eq, self.dpi);
                                let color_str =
                                ...
                                // HWP `baseLine` 속성(=height에 대한 % 0..100)이 있으면 그것을 우선 사용한다.
                                // 한컴은 인라인 첨자 식에 baseLine≈85, 디스플레이 식에 ≈58 을 기록한다.
                                // 속성 누락 시 rhwp 자체 layout 기반 baseline 으로 폴백.
                        ...
                        if let (Some(p), Some(bdc)) = (para, bin_data_content) {
                            if let Some(Control::Table(t)) = p.controls.get(tac_ci) {
                                if t.common.treat_as_char {
                                    ...
                                    if std::env::var("RHWP_LAYOUT_DEBUG").is_ok() {
                                        eprintln!("LAYOUT_TAC_TABLE pi={} y={:.2} baseline={:.2} om_b={:.2} table_h={:.2} table_y={:.2}",
                                            para_index, y, baseline, om_bottom, table_h, table_y);
                                    }
                                    self.layout_table(
                                        tree,
                                        col_node,
            ...
            if use_stored_vp {
                // 2026-05-19: line bottom = stored vertpos + stored line_height.
                // 모든 paragraph 에 stored vp 기반 누적. (이전: has_inline_tac_table 만)
                y = y_para_origin
                    + hwpunit_to_px(
                        comp_line.vertical_pos - stored_vp_base + comp_line.line_height,
                        self.dpi,
                    );
        ...
        if para_border_fill_id > 0 && cell_ctx.is_none() {
            ...
            if bg_height > 0.0 {
                ...
                let (box_x, box_w) = if let Some((ox, ow)) = self.border_box_override.get() {
                    ...
                    (
                        col_area.x + box_margin_left,
                        col_area.width - box_margin_left - box_margin_right,
                    )
                };
        ...
        if std::env::var("RHWP_LAYOUT_DEBUG").is_ok() {
            eprintln!(
                "LAYOUT_PARA pi={} return y={:.2} (delta={:.2})",
                para_index,
                y,
                y - y_start
            );
