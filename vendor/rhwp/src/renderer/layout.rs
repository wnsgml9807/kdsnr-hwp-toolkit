            ...
            if has_table {
                for (ci, ctrl) in para.controls.iter().enumerate() {
                    if let Control::Table(t) = ctrl {
                        ...
                        let line_anchor_offset = if matches!(
                            t.common.text_wrap,
            ...
            if let Some(Control::Table(t)) = para.controls.get(control_index) {
                ...
                if renders_outside_body {
                    ...
                    if std::env::var("RHWP_LAYOUT_DEBUG").is_ok() {
                        eprintln!("LAYOUT_TABLE_ITEM pi={} ci={} is_tac={} y_before={:.2} table_y_start={:.2} y_after_table={:.2} para_y_for_table={:.2}",
                            para_index, control_index, is_tac, y_before, table_y_start, y_offset, para_y_for_table);
                    }
                }
                ...
                if is_tac {
                    ...
                    if tac_idx_current < tac_count_total && !has_non_tac_between {
                        ...
                        if let Some(seg) = para.line_segs.get(seg_idx) {
                            ...
                            if std::env::var("RHWP_LAYOUT_DEBUG").is_ok() {
                                eprintln!("LAYOUT_TABLE_ITEM pi={} TAC last_clamp seg_idx={} vpos={} lh={} para_y_for_table={:.2} line_end={:.2} table_y_end={:.2} clamped={:.2} max_corr={:.2} y_before_clamp={:.2}",
                                    para_index, seg_idx, seg.vertical_pos, seg.line_height,
                                    para_y_for_table, line_end, table_y_end, clamped, max_correction, y_offset);
                            }
