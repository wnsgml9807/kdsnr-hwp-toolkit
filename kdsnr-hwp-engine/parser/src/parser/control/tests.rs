...
#[test]
fn test_parse_table_cell_explicit_padding_enables_inner_margin() {
    let mut table_data = Vec::new();
    table_data.extend_from_slice(&0u32.to_le_bytes()); // attr
    ...
    table_data.extend_from_slice(&0i16.to_le_bytes()); // padding_top
    table_data.extend_from_slice(&0i16.to_le_bytes()); // padding_bottom
    table_data.extend_from_slice(&500i16.to_le_bytes()); // row height
    table_data.extend_from_slice(&1u16.to_le_bytes()); // border_fill_id

    let mut cell_data = Vec::new();
    cell_data.extend_from_slice(&0u16.to_le_bytes()); // n_paragraphs
    cell_data.extend_from_slice(&0u32.to_le_bytes()); // list_attr
    cell_data.extend_from_slice(&0u16.to_le_bytes()); // width_ref/property bits
    cell_data.extend_from_slice(&0u16.to_le_bytes()); // col
    cell_data.extend_from_slice(&0u16.to_le_bytes()); // row
    cell_data.extend_from_slice(&1u16.to_le_bytes()); // col_span
    cell_data.extend_from_slice(&1u16.to_le_bytes()); // row_span
    cell_data.extend_from_slice(&10000u32.to_le_bytes()); // width
    cell_data.extend_from_slice(&5000u32.to_le_bytes()); // height
    cell_data.extend_from_slice(&141i16.to_le_bytes()); // padding_left
    cell_data.extend_from_slice(&141i16.to_le_bytes()); // padding_right
    cell_data.extend_from_slice(&141i16.to_le_bytes()); // padding_top
    cell_data.extend_from_slice(&141i16.to_le_bytes()); // padding_bottom
    cell_data.extend_from_slice(&1u16.to_le_bytes()); // border_fill_id
    ...
    if let Control::Table(table) = ctrl {
        ...
        assert_eq!(table.cells[0].padding.right, 141);
        assert_eq!(table.cells[0].padding.top, 141);
        assert_eq!(table.cells[0].padding.bottom, 141);
        assert!(
            !table.cells[0].apply_inner_margin,
            "cell margin payload must not be promoted to hasMargin without the source property bit"
        );
    } else {
        panic!("Expected Table control");
    }
