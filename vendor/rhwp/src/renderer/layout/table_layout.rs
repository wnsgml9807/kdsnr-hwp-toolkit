...
impl LayoutEngine {
    ...
    pub(crate) fn layout_table(
        ...
        // 캡션은 depth 와 무관하게 처리 (nested 데이터표의 caption — 예: Q20 의 "(단위: %)"
        // — 도 outer 박스 안에서 정상 렌더링되어야 한다).
        let caption_height = self.calculate_caption_height(&table.caption, styles);
        let caption_spacing = table
            .caption
            .as_ref()
        ...
        if std::env::var("RHWP_LAYOUT_DEBUG").is_ok() {
            eprintln!("LAYOUT_TABLE pi={:?} ci={:?} y_start={:.2} table_y={:.2} table_h={:.2} (delta_y={:.2}) inline_override={} depth={} para_y={:?}",
                table_meta.map(|(p,_)|p), table_meta.map(|(_,c)|c),
                y_start, table_y, table_height, table_y - y_start, inline_x_override.is_some(), depth, para_y);
        }
        ...
        if let Some(ref caption) = table.caption {
            {
                ...
                let (cap_x, cap_w, cap_y) = match caption.direction {
                    CaptionDirection::Top => (
                        table_x,
                        table_width,
                        table_y - caption_height - caption_spacing,
                    ),
    ...
    pub(crate) fn resolve_row_heights(
        ...
        let has_square_floating_picture = table.cells.iter().any(|cell| {
        ...
        if row_count == 1
            && has_square_floating_picture
            && table.common.height > 0
            && table.common.height < 0x8000_0000
        {
            let explicit_h = hwpunit_to_px(table.common.height as i32, self.dpi);
            if explicit_h > 0.0 {
                row_heights[0] = explicit_h;
            }
        }
        row_heights
    }
    ...
    pub(crate) fn calc_cell_paragraphs_content_height(
        ...
        paragraphs
            ...
            .enumerate()
            .map(|(pidx, p)| {
                // G-W-3b: cell paragraph 도 env-gated reflow
                let comp =
                    crate::renderer::composer::compose_paragraph_with_reflow(p, styles, self.dpi);
                let para_style = styles.para_styles.get(p.para_shape_id as usize);
                let line_based_h =
                    self.calc_para_lines_height(&comp.lines, pidx, cell_para_count, para_style);
                let nested_h: f64 = p
                    .controls
                    .iter()
                    .filter_map(|ctrl| {
                        if let Control::Table(t) = ctrl {
                            Some(self.calc_nested_table_height(t, styles))
                        } else {
                            None
                        }
                    })
                    .sum();
                let non_inline_h: f64 = p
                    .controls
                    .iter()
                    .filter_map(|ctrl| match ctrl {
                        ...
                        {
                            Some(hwpunit_to_px(shape.common().height as i32, self.dpi))
                        }
                        Control::Equation(eq)
                            if !eq.common.treat_as_char
                                && matches!(eq.common.text_wrap, TextWrap::TopAndBottom) =>
                        {
                            Some(hwpunit_to_px(eq.common.height as i32, self.dpi))
                        }
                        _ => None,
                    })
                    .sum();
                let line_based_h = line_based_h + non_inline_h;
                if nested_h > 0.0 {
                    ...
                    let spacing_after = if pidx + 1 != cell_para_count {
                        para_style.map(|s| s.spacing_after).unwrap_or(0.0)
                    } else {
                        0.0
                    };
                    line_based_h.max(spacing_before + nested_h + spacing_after)
                } else {
                    line_based_h
                }
            })
    ...
    pub(crate) fn resolve_cell_padding(
        ...
        if std::env::var("RHWP_PAD_DUMP").is_ok() {
            eprintln!(
                "PAD cell({},{}) aim={} cell.pad={{l={},r={},t={},b={}}} tbl.pad={{l={},r={},t={},b={}}} → final px=({:.2},{:.2},{:.2},{:.2})",
                cell.col, cell.row, cell.apply_inner_margin,
    ...
    fn layout_table_cells(
        ...
        for (cell_idx, cell) in table.cells.iter().enumerate() {
            ...
            // 단, 셀이 자체 margin을 명시한 경우에는 그 padding 자체가 양식의 일부다.
            // `<보기>`류 표에서 이 값을 줄이면 항목들이 테두리에 붙고, 한컴이 저장한
            // lineSeg 상대 위치와도 어긋난다.
            let has_explicit_cell_margin = cell.apply_inner_margin
                || table.padding.left > 0
                || table.padding.right > 0
                || cell.padding.left > table.padding.left
                || cell.padding.right > table.padding.right;
            if !has_explicit_cell_margin {
                    ...
                    for ctrl in &para.controls {
                        match ctrl {
                            ...
                            Control::Shape(shape) => {
                                ...
                                } else if matches!(shape.common().text_wrap, TextWrap::TopAndBottom)
                                {
                                    text_height += shape_h;
                                }
                            }
                            Control::Equation(eq) => {
                                ...
                                } else if matches!(eq.common.text_wrap, TextWrap::TopAndBottom) {
                                    text_height += eq_h;
                                }
                            }
                            Control::Table(t) => {
            ...
            if cell.text_direction != 0 {
                ...
                let mut active_right_square_obstacle: Option<(f64, f64)> = None; // (left_x, bottom_y)
                    ...
                    .enumerate()
                {
                    ...
                    let mut para_area = inner_area;
                    let mut consumed_square_obstacle = false;
                    if let Some((obstacle_left, obstacle_bottom)) = active_right_square_obstacle {
                        if para_y < obstacle_bottom - 0.5 {
                            para_area.width = (obstacle_left - para_area.x).max(0.0);
                            consumed_square_obstacle = true;
                        } else {
                            active_right_square_obstacle = None;
                        }
                    }

                    // cell 내부 단락이라도 inline-table (분수 등) 을 가지면 텍스트가 사라지지 않도록
                    // page-level 경로와 동일하게 `layout_inline_table_paragraph` 로 routing 해야 한다.
                    // 조건: 적어도 하나의 treat-as-char Table 이 inline 으로 판정되고,
                    //       Picture/Shape/Equation TAC 컨트롤은 섞여있지 않을 때 (해당 함수는 Table 만 처리).
                    let inner_width_hwpunit =
                        super::super::px_to_hwpunit(inner_width, self.dpi) as i32;
                    let route_inline_table = {
                        let has_inline_table = para.controls.iter().any(|c| {
                            matches!(c,
                                Control::Table(t) if t.common.treat_as_char
                                    && crate::renderer::height_measurer::is_tac_table_inline(
                                        t, inner_width_hwpunit, &para.text, &para.controls,
                                    )
                            )
                        });
                        let has_other_tac = para.controls.iter().any(|c| match c {
                            Control::Picture(p) => p.common.treat_as_char,
                            Control::Shape(s) => s.common().treat_as_char,
                            Control::Equation(_) => true,
                            _ => false,
                        });
                        has_inline_table && !has_other_tac
                    };

                    if route_inline_table {
                        let is_last_para = cp_idx + 1 == composed_paras.len();
                        para_y = self.layout_inline_table_paragraph(
                            tree,
                            &mut cell_node,
                            para,
                            Some(composed),
                        ...
                        let has_visible_text = !para.text.trim().is_empty();
                        if has_visible_text {
                    ...
                    if !has_table_ctrl {
                        ...
                        if consumed_square_obstacle {
                            if let Some((_, obstacle_bottom)) = active_right_square_obstacle {
                                if para_y < obstacle_bottom {
                                    para_y = obstacle_bottom;
                                }
                            }
                            active_right_square_obstacle = None;
                        }
                        ...
                        match ctrl {
                            Control::Picture(pic) => {
                                if pic.common.treat_as_char {
                                    ...
                                    let is_square_float =
                                        matches!(pic.common.text_wrap, TextWrap::Square);
                                    let pic_anchor_y = if is_square_float {
                                    ...
                                    let cell_area = LayoutRect {
                                        y: pic_anchor_y,
                                        height: (inner_area.height - (pic_anchor_y - inner_area.y))
                                            .max(0.0),
                                        ..inner_area
                                    };
                                    ...
                                    let pic_rect = LayoutRect {
                                        x: pic_x,
                                        y: pic_y,
                                        width: pic_w,
                                        height: pic_h,
                                    };
                                    self.layout_picture_at_rect(
                                        tree,
                                        &mut cell_node,
                                        pic,
                                        &pic_rect,
                                    ...
                                    if is_square_float {
                                        ...
                                        active_right_square_obstacle =
                                            Some((pic_x - margin_left, pic_y + pic_h));
                                    } else {
                                        para_y += pic_h;
                                    }
                                }
                    ...
                    //
                    // 표 컨트롤이 없는 일반 셀 문단은 위의 layout_composed_paragraph()
                    // 경로가 이미 TAC 앞/뒤 본문 텍스트를 모두 렌더링한다. 여기서 다시
                    // emit 하면 treatAsChar 글상자 뒤 첫 글자가 중복된다.
                    if prev_tac_text_pos > 0 && has_table_ctrl {
