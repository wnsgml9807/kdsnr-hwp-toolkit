...
impl TextMeasurer for EmbeddedTextMeasurer {
    fn estimate_text_width(&self, text: &str, style: &TextStyle) -> f64 {
        ...
        let char_width = |i: usize| -> f64 {
            ...
            let base_w_raw = if let Some(w) = measure_char_width_embedded_with_raw(
                &style.font_family,
                &style.raw_font_family,
                style.bold,
                style.italic,
                c,
                font_size,
        ...
        for i in 0..char_count {
            ...
            if c == '\t' {
                ...
                // HWPX raw type (0/1/2/3) 가 우리 match 의 _/1/2/_ 분기와 정확히 매핑.
                if tab_char_idx < style.inline_tabs.len() {
                    let ext = &style.inline_tabs[tab_char_idx];
                    let tab_width_px = ext[0] as f64 * 96.0 / 7200.0;
                    let inline_type = ext[2];
                    // 한컴 동작 (probe + 실측 기반):
                    //  1) ParaShape.tab_def 의 stop 중 abs_x 이후 첫 stop 이 있으면 그 stop 의
                    //     position + tab_type 으로 정렬 (Q20 분수 마커 케이스: TabDef[22].stop[0]
                    //     = pos 3200, type=1 (RIGHT) → `*`/`**`/`***` right-edge 정렬).
                    //  2) stop 없으면 inline.width 만큼 advance + inline.type 정렬.
                    //  3) autoTabRight=1 케이스는 has_custom_tabs 분기에서 별도 처리.
                    let abs_x = style.line_x_offset + total;
                    let next_stop = style
                        .tab_stops
                        .iter()
                        .find(|ts| ts.position > abs_x + 0.5)
                        .map(|ts| (ts.position, ts.tab_type));
                    let (tab_target, tab_type): (f64, u16) =
                        if let Some((pos_abs, stop_type)) = next_stop {
                            (pos_abs - style.line_x_offset, stop_type as u16)
                        } else {
                            (total + tab_width_px, inline_type as u16)
                        };
                    let seg_w_after =
                        measure_segment_from(&chars, &cluster_len, i + 1, &char_width);
                    match tab_type {
                        1 | 3 => {
                            // 1 = RIGHT, 3 = DECIMAL — 한컴 시험지 패턴에서 type=3 는
                            // 다음 tab/줄끝까지의 text 를 right-edge 정렬 (소수점이 없는
                            // 본문 텍스트면 right 와 동일 동작). 한컴 GT 의 "사회탐구 영역" /
                            // "수학 영역" 헤더의 우측 정렬이 이 경로.
                            total = (tab_target - seg_w_after).max(total);
                        }
                        2 => {
                            total = (tab_target - seg_w_after / 2.0).max(total);
                        }
                        _ => {
                    ...
                    match tab_type {
                        1 | 3 => {
                            // 1 = 오른쪽, 3 = 소수점 (한컴 시험지에서 right 와 동일 동작)
                            let seg_w =
                                measure_segment_from(&chars, &cluster_len, i + 1, &char_width);
        ...
        for i in 0..char_count {
            ...
            if c == '\t' {
                ...
                if tab_char_idx < style.inline_tabs.len() {
                    ...
                    let tab_width_px = ext[0] as f64 * 96.0 / 7200.0;
                    let inline_type = ext[2];
                    // 위 measure_text_width 와 동일한 결정 트리: stop 있으면 stop.type 사용.
                    ...
                    let (tab_target, tab_type): (f64, u16) =
                        if let Some((pos_abs, stop_type)) = next_stop {
                            ...
                            (x + tab_width_px, inline_type as u16)
                        };
                    let seg_w_after = {
                        let seg_start = if tab_type == 1 || tab_type == 3 {
                            let mut s = i + 1;
                    ...
                    match tab_type {
                        1 | 3 => {
                            // RIGHT / DECIMAL — 한컴 시험지 헤더 ("사회탐구 영역" 등) 가 type=3 사용
                            x = (tab_target - seg_w_after).max(x);
                        }
                        2 => {
                            x = (tab_target - seg_w_after / 2.0).max(x);
                        }
                        _ => {
                            x = tab_target.max(x);
                        }
...
pub(crate) fn resolved_to_text_style(
    ...
    if let Some(cs) = styles.char_styles.get(char_style_id as usize) {
        TextStyle {
            ...
            raw_font_family: cs.raw_font_family_for_lang(lang_index).to_string(),
            ...
            any_lang_heavy_face: cs
                .font_families
                .iter()
                .any(|f| crate::renderer::style_resolver::is_heavy_display_face(f)),
        }
    } else {
        TextStyle::default()
    }
...
/// 폰트 메트릭으로 문자 폭 측정 (em 단위 → px 변환).
///
/// 우선순위:
/// 1. **fontdb 의 ttf 에서 hmtx 직접 측정** (런타임, 정확). native 환경 전용.
///    - resolve_font_substitution 이 family 명을 fontdb 매칭 family 로 평탄화한 후
///      `font_runtime_metrics::measure_char_advance_em` 호출.
/// 2. **사전 측정 메트릭 테이블** (`font_metrics_data`, HANCOM_FONT_METRICS 포함).
///    fontdb 매칭 실패 시 fallback.
/// 3. None 반환 (호출자가 처리).
fn measure_char_width_embedded(
    ...
    measure_char_width_embedded_with_raw(font_family, "", bold, italic, c, font_size)
}

/// `measure_char_width_embedded` 의 raw face 인자 확장 버전.
///
/// 한컴 native 와 byte-eq 한 advance 를 얻으려면 HFT cache lookup 을 **raw face
/// name** ("신명 중명조" 등) 으로 해야 한다. 현재 호출처는 대부분 substituted
/// name ("HCR Batang") 만 전달 → cache miss → fontdb hmtx 폴백 → advance 미스매치.
/// raw_name 이 비어있지 않으면 우선 시도하고, miss 시 substituted name 으로 fallback.
#[allow(dead_code)]
fn measure_char_width_embedded_with_raw(
    font_family: &str,
    raw_name: &str,
    bold: bool,
    italic: bool,
    ...
    let raw_primary = raw_name.split(',').next().unwrap_or(raw_name).trim();

    // === 0순위: kdsnr-hft cache — raw face name 우선, miss 시 substituted ===
    // 한컴 native 와 byte-eq 한 advance. raw_name 이 있으면 ("신명 중명조" 등)
    // 그것으로 먼저 HFT cache lookup. raw 비어있거나 miss 면 substituted 로 시도.
    #[cfg(not(target_arch = "wasm32"))]
    {
        let try_names: [&str; 2] = [raw_primary, primary_name];
        for name in try_names.iter() {
            if name.is_empty() {
                continue;
            }
            if let Some(em) = crate::renderer::kdsnr_hft_global::advance_em(name, c) {
                let is_halfwidth_punct = matches!(c, '\u{2018}'..='\u{2027}' | '\u{00B7}');
                let em_used = if is_halfwidth_punct && em >= 1.0 {
                    0.5
                } else {
                    em
                };
                let actual_px = em_used * font_size;
                let hwp = (actual_px * 75.0) as i32;
                return Some(hwp as f64 / 75.0);
            }
        }
    }

    // === 1순위: fontdb 의 ttf 에서 직접 hmtx 측정 ===
    #[cfg(not(target_arch = "wasm32"))]
    {
        if let Some(em) = crate::renderer::font_runtime_metrics::measure_char_advance_em(
            ...
            // HWP 반각 처리: 한 컴이 반각으로 처리하는 구두점/기호.
            // glyph advance 가 full-em 인데 한 컴이 반각 강제하는 케이스.
            let is_halfwidth_punct = matches!(c, '\u{2018}'..='\u{2027}' | '\u{00B7}');
            let em_used = if is_halfwidth_punct && em >= 1.0 {
                0.5
            } else {
            ...
            let actual_px = em_used * font_size;
            // HWPUNIT 양자화 (한컴 정수 나눗셈과 일치)
            let hwp = (actual_px * 75.0) as i32;
            return Some(hwp as f64 / 75.0);
        }
    }

    // === 2순위: 사전 측정 메트릭 테이블 (font_metrics_data) ===
...
pub(crate) fn estimate_text_width_unrounded(text: &str, style: &TextStyle) -> f64 {
    ...
    let char_width = |i: usize| -> f64 {
        ...
        // raw_font_family 전달 → HFT cache (한컴 native advance) 우선. mismatch 줄임.
        let base_w_raw = if let Some(w) = measure_char_width_embedded_with_raw(
            &style.font_family,
            &style.raw_font_family,
            style.bold,
...
pub fn compute_char_positions(text: &str, style: &TextStyle) -> Vec<f64> {
