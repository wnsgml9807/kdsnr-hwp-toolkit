...
pub struct ParaShape {
    ...
    pub para_level: u8,
    /// hh:paraPr 의 `condense` 속성 (0~75 %, 한 줄 강제 축소율).
    /// HWP 5.0 binary spec 에는 없는 hwpx-전용 속성. 한컴은 hwpx export 시
    /// 거의 모든 paragraph 에 50 을 박는다 (빈 paragraph 등 예외는 0).
    pub condense: u8,
    /// hh:align 의 `vertical` 속성 (글자 단위 세로 정렬).
    /// 0=BASELINE, 1=WORD, 2=TOP, 3=CENTER, 4=BOTTOM.
    /// HWP 5.0 binary 에서는 attr1 bit 20~21 에 인코딩 (2비트 → 0~3).
    /// 0=BASELINE, 1=CENTER, 2=BOTTOM 정도가 실측 매핑. attr1 → 출력 enum 변환은
    /// serializer 에서 처리.
